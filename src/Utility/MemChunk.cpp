...
//              arbitrary sized chunks of memory / binary data.
//
...
bool MemChunk::reSize(uint32_t new_size, bool preserve_data)
{
	...
	if (new_size == 0)
	{
		Log::error(1, "MemChunk::reSize: new_size cannot be 0");
		return false;
	}
...
// -----------------------------------------------------------------------------
// Loads a chunk of memory from [start] of size [len] into the MemChunk.
...
// -----------------------------------------------------------------------------
// Allocates [size] bytes of data and returns it, or nullptr if the allocation
// failed.
// If [set_data] is true, the MemChunk data will also be set to the allocated
// data if successful, or set to nullptr and the size set to 0 if allocation
// failed.
// -----------------------------------------------------------------------------
