...
{
	sub_polys_.emplace_back();
	vbo_update_ = 2;
}

Polygon2D::SubPoly* Polygon2D::getSubPoly(unsigned index)
{
	if (index >= sub_polys_.size())
		return nullptr;
	else
		return &sub_polys_[index];
}
...
void Polygon2D::removeSubPoly(unsigned index)
{
	...
	sub_polys_.erase(sub_polys_.begin() + index);
	vbo_update_ = 2;
}
...
unsigned Polygon2D::writeToVBO(unsigned offset, unsigned index)
{
	...
	for (auto& poly : sub_polys_)
	{
		...
		glBufferSubData(GL_ARRAY_BUFFER, ofs, poly.vertices.size() * 20, poly.vertices.data());

		// Update the subpoly vbo offset
		poly.vbo_offset = ofs;
		poly.vbo_index  = i;
		ofs += poly.vertices.size() * 20;
...
void Polygon2D::updateVBOData()
{
	// Go through subpolys
	for (auto& poly : sub_polys_)
		glBufferSubData(GL_ARRAY_BUFFER, poly.vbo_offset, poly.vertices.size() * 20, poly.vertices.data());
...
void Polygon2D::renderVBO(bool colour)
{
	...
	for (auto& poly : sub_polys_)
		glDrawArrays(GL_TRIANGLE_FAN, poly.vbo_index, poly.vertices.size());
}
...
int PolygonSplitter::addEdge(int v1, int v2)
{
	...
	// Add edge to list
	edges_.push_back({ v1, v2, true, false, false, -1 });
...
bool PolygonSplitter::detectUnclosed()
{
	...
	for (int i_ev : end_verts)
	{
		Vertex& ev = vertices_[i_ev];

		// Check all the edges coming out of this vertex,
		// and see if any go into another 'unattacted' vertex
		for (int i_edge : ev.edges_in)
		{
			Edge& edge = edges_[i_edge];

			for (int i_sv : start_verts)
				if (edge.v1 == i_sv && edge.v2 == i_ev)
					flipEdge(i_edge); // Flip the edge
		}
	}
...
bool PolygonSplitter::tracePolyOutline(int edge_start)
{
	...
	for (a = 0; a < 100000; a++)
	{
		...
		if (next < 0)
		{
			for (int i_edge : poly.edges)
				edges_[i_edge].inpoly = false;
...
struct VertDist
{
	int    index;
	double distance;
	VertDist(int index, double distance) : index{ index }, distance{ distance } {}
	bool operator<(const VertDist& right) const { return distance < right.distance; }
};
bool PolygonSplitter::splitFromEdge(int splitter_edge)
{
	...
	for (auto& edge : edges_)
	{
		...
		if (edge.v1 == closest || edge.v2 == closest || edge.v1 == v2 || edge.v2 == v2
			|| !edge.ok)
			continue;
		...
		if (MathStuff::linesIntersect(
				...
				fseg2_t(vertices_[edge.v1], vertices_[edge.v2]),
				pointi))
		{
			intersect = true;
			break;
	...
	for (auto& sorted_vert : sorted_verts)
	{
		int     index = sorted_vert.index;
		...
		{
			// Ignore edge if adjacent to the vertices we are looking at
			if (edge.v1 == index || edge.v2 == index || edge.v1 == v2 || edge.v2 == v2
				|| !edge.ok)
				continue;

			// Intersection test
			if (MathStuff::linesIntersect(
					fseg2_t(vertices_[v2], vert), fseg2_t(vertices_[edge.v1], vertices_[edge.v2]), pointi))
			{
				intersect = true;
				break;
			}
...
bool PolygonSplitter::buildSubPoly(int edge_start, Polygon2D::SubPoly* poly)
{
	...
	if (verts.size() >= 3)
	{
		...
		for (int vert : verts)
			poly->vertices.emplace_back(vertices_[vert].x, vertices_[vert].y, 0.f);
