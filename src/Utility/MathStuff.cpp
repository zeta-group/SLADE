...
// -----------------------------------------------------------------------------
double MathStuff::distance(const fpoint2_t& p1, const fpoint2_t& p2)
{
...
// Returns the distance between p1 and p2
// -----------------------------------------------------------------------------
double MathStuff::distance3d(const fpoint3_t& p1, const fpoint3_t& p2)
{
...
// -----------------------------------------------------------------------------
double MathStuff::lineSide(const fpoint2_t& point, const fseg2_t& line)
{
...
// -----------------------------------------------------------------------------
fpoint2_t MathStuff::closestPointOnLine(const fpoint2_t& point, const fseg2_t& line)
{
	// Get line length
...
// -----------------------------------------------------------------------------
// Returns the shortest distance between the point at [x,y] and the line from
// [x1,y1] to [x2,y2]
// -----------------------------------------------------------------------------
double MathStuff::distanceToLine(const fpoint2_t& point, const fseg2_t& line)
{
...
// -----------------------------------------------------------------------------
double MathStuff::distanceToLineFast(const fpoint2_t& point, const fseg2_t& line)
{
	// Calculate intersection point
	fpoint2_t i = closestPointOnLine(point, line);
...
// -----------------------------------------------------------------------------
bool MathStuff::linesIntersect(const fseg2_t& l1, const fseg2_t& l2, fpoint2_t& out)
{
	...
	return min(l1.x1(), l1.x2()) <= out.x && out.x <= max(l1.x1(), l1.x2()) && min(l1.y1(), l1.y2()) <= out.y
...
// [seg1 -> seg2]
// -----------------------------------------------------------------------------
double MathStuff::distanceRayLine(const fpoint2_t& r1, const fpoint2_t& r2, const fpoint2_t& s1, const fpoint2_t& s2)
{
	...
	return (u_ray >= 0 && u_line >= 0 && u_line <= 1) ? u_ray : -1;
}

// -----------------------------------------------------------------------------
// Returns the angle between the 2d points [p1], [p2] and [p3]
// -----------------------------------------------------------------------------
double MathStuff::angle2DRad(const fpoint2_t& p1, const fpoint2_t& p2, const fpoint2_t& p3)
{
...
// -----------------------------------------------------------------------------
// Rotates [point] around [origin] by [angle] (in degrees) and returns the newly
// rotated point
// -----------------------------------------------------------------------------
fpoint2_t MathStuff::rotatePoint(const fpoint2_t& origin, const fpoint2_t& point, double angle)
{
	// Translate to origin
...
// vector
// -----------------------------------------------------------------------------
fpoint3_t MathStuff::rotateVector3D(const fpoint3_t& vector, const fpoint3_t& axis, double angle)
{
	fpoint3_t rvec;

	// Calculate the sine and cosine of the angle once
	auto crot = (float)cos(angle);
	auto srot = (float)sin(angle);
...
// -----------------------------------------------------------------------------
double MathStuff::distanceRayPlane(const fpoint3_t& r_o, const fpoint3_t& r_v, const Plane& plane)
{
...
// -----------------------------------------------------------------------------
// Returns true if the given rectangle [box] intersects with the given line
// segment [line].
// Box values must be from min to max.
// Taken from http://stackoverflow.com/a/100165
// -----------------------------------------------------------------------------
bool MathStuff::boxLineIntersect(const frect_t& box, const fseg2_t& line)
{
...
// -----------------------------------------------------------------------------
Plane MathStuff::planeFromTriangle(const fpoint3_t& p1, const fpoint3_t& p2, const fpoint3_t& p3)
{
	fpoint3_t v1 = p3 - p1;
	fpoint3_t v2 = p2 - p1;
	v1.normalize();
