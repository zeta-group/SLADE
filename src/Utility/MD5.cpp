...
CONSOLE_COMMAND(calcmd5, 0, false)
{
	...
	{
		md5.init();
		md5.update(selection[a]->rawData(), selection[a]->size());
		md5.finalize();
...
CONSOLE_COMMAND(test_md5, 0, false)
{
	...
	if (archive)
	{
		...
		{
			md5.init();
			md5.update(entry->rawData(), entry->size());
			md5.finalize();
		}
		auto elapsed = App::runTimer() - start;
		Log::console(fmt::format("Took {}ms", elapsed));
	}
}
...
CONSOLE_COMMAND(test_crc, 0, false)
{
	...
	if (archive)
	{
		...
		auto start = App::runTimer();
		for (auto& entry : entries)
			Misc::crc(entry->rawData(), entry->size());
		auto elapsed = App::runTimer() - start;
		Log::console(fmt::format("Took {}ms", elapsed));
	}
}
