...
public:
	...
	string selectedName() const { return text_name_->GetValue().ToStdString(); }
...
{
	string name      = StrUtil::beforeFirst(path, '/');
	string path_rest = StrUtil::afterFirst(path, '/');
	...
	while (child.IsOk())
	{
		if (tree->GetItemText(child).CmpNoCase(name) == 0)
			break;
...
// -----------------------------------------------------------------------------
void ScriptManagerWindow::showDocs(string_view url)
{
	...
	// Load page if set
	if (!found || !url.empty())
		webview_docs_->LoadURL(docs_url + "/" + WxUtils::stringFromView(url));
...
bool ScriptManagerWindow::handleAction(string_view id)
{
	...
	if (id == "scrm_rename")
	{
		...
		{
			string name =
				wxGetTextFromUser("Enter a new name for the script", "Rename Script", script->name).ToStdString();
