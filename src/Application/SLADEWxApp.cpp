...
private:
	string stack_trace_ = "Stack Trace:\n";
	string top_level_;
};
...
public:
	SLADECrashDialog(SLADEStackTrace& st) : wxDialog(wxTheApp->GetTopWindow(), -1, "SLADE Application Crash")
	{
		...
		// System info
		auto gl_info = OpenGL::systemInfo();
	...
	void onThreadUpdate(wxThreadEvent& e)
	{
		...
		else
		{
			...
			btn_exit_->Enable();
			wxMessageBox(
				"The crash report failed to send. Please either try again or click 'Exit SLADE' to exit without "
				"sending.",
				"Failed to Send");
		}
	}
...
void SLADEWxApp::onVersionCheckCompleted(wxThreadEvent& e)
{
	...
	auto info = StrUtil::split(e.GetString().ToStdString(), '\n');
	...
	// Get version numbers
	auto version_stable = std::stoi(info[0]);
...
CONSOLE_COMMAND(quit, 0, true)
{
	...
	{
		if (StrUtil::lower(args[a]) == "nosave")
			save_config = false;
	}
