...
{
// -----------------------------------------------------------------------------
// Returns the path for [exe] with some extra processing on macos
// -----------------------------------------------------------------------------
...
public:
	...
	string name() const { return text_name_->GetValue().ToStdString(); }
	string params() const { return text_params_->GetValue().ToStdString(); }
	...
	SDialog(parent, "Run", "run", 500, 400)
{
	...
	text_extra_params_ = new wxTextCtrl(this, -1, run_last_extra.value);
	...
	wxDialog::SetMinSize(wxSize(UI::scalePx(500), UI::scalePx(400)));
	wxDialog::Layout();
	CenterOnParent();
...
string RunDialog::selectedCommandLine(Archive* archive, string_view map_name, string_view map_file) const
{
	...
	if (exe)
	{
		...
		else
		{
			...
			{
				string mn = StrUtil::lower(map_name);
...
string RunDialog::selectedExeDir() const
{
	auto exe = Executables::gameExe(choice_game_exes_->GetSelection());
	if (exe)
		return StrUtil::Path::pathOf(exe->path, false).to_string();
...
{
	string name = wxGetTextFromUser("Enter a name for the game executable").ToStdString();
...
void RunDialog::onBtnEditConfig(wxCommandEvent& e)
{
	...
	if (dlg.ShowModal() == wxID_OK)
	{
		string nname               = dlg.name().empty() ? exe->configs[index].first : dlg.name();
		exe->configs[index].first  = nname;
		exe->configs[index].second = dlg.params();
		choice_config_->SetString(index, nname);
	}
}
...
void RunDialog::onBtnRun(wxCommandEvent& e)
{
	...
	// Update cvars
	run_last_extra  = text_extra_params_->GetValue().ToStdString();
