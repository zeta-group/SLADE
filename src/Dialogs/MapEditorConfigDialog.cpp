...
//              port, and resource archives.
//              Also can show a map list and preview
//
...
// NewMapDialog Class
//
// A dialog for creating a new map - select map format and name
// -----------------------------------------------------------------------------
...
public:
		...
		wxDialog(parent, -1, "New Map")
	{
		...
		sizer->Add(new wxStaticText(this, -1, "Map Name:"), { 0, 0 }, wxDefaultSpan, wxALIGN_CENTER_VERTICAL);
		sizer->Add(cbo_mapname_, { 0, 1 }, wxDefaultSpan, wxEXPAND);
		...
		sizer->Add(new wxStaticText(this, -1, "Map Format:"), { 1, 0 }, wxDefaultSpan, wxALIGN_CENTER_VERTICAL);
		sizer->Add(choice_mapformat_, { 1, 1 }, wxDefaultSpan, wxEXPAND);
		...
		{
			if (Game::mapFormatSupported(map_type, game.to_string(), port.to_string()))
			{
	...
	string mapName() { return cbo_mapname_->GetValue().ToStdString(); }

	string mapFormat() { return choice_mapformat_->GetStringSelection().ToStdString(); }
...
void MapEditorConfigDialog::populateMapList()
{
	...
	for (auto& map : maps_)
	{
		...
		if (map.format == MAP_DOOM64)
			...
			fmt = "U";
		else if (map.format == MAP_UNKNOWN)
			fmt = "?";
...
Archive::MapDesc MapEditorConfigDialog::selectedMap()
{
	if (creating_)
	{
		...
		if (dlg.ShowModal() == wxID_OK)
		{
			...
			mdesc.name = dlg.mapName();

			// Get selected map format
			int map_format = MAP_DOOM;
			for (uint8_t map_type = 0; map_type < MAP_UNKNOWN; map_type++)
				if (dlg.mapFormat() == MAP_TYPE_NAMES[map_type])
				{
					map_format = map_type;
					break;
				}
