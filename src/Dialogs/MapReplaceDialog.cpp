...
void TextureReplacePanel::doReplace(Archive* archive)
{
	size_t count = ArchiveOperations::replaceTextures(
		archive,
		text_from_->GetValue().ToStdString(),
		text_to_->GetValue().ToStdString(),
		cb_floor_->GetValue(),
	...
	wxDialog(parent, -1, "Replace In Maps", wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
	...
	btn_replace_->Bind(wxEVT_BUTTON, [&](wxCommandEvent&) {
		switch (stc_tabs_->GetSelection())
		{
		case 0: panel_thing_->doReplace(archive_); break;   // Thing types
		case 1: panel_special_->doReplace(archive_); break; // Specials
		case 2: panel_texture_->doReplace(archive_); break; // Textures
		default: break;
		}
	});
}
