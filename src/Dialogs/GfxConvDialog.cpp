...
// Returns true if the selected format was valid for the next image, false
// otherwise
// -----------------------------------------------------------------------------
bool GfxConvDialog::nextItem()
{
	...
	for (auto format : all_formats)
	{
		...
		if (format->canWrite(items_[current_item_].image))
		{
...
void GfxConvDialog::setupLayout()
{
	...
	btn_skip_->Bind(wxEVT_BUTTON, [&](wxCommandEvent&) { nextItem(); });
	btn_skip_all_->Bind(wxEVT_BUTTON, [&](wxCommandEvent&) { Close(true); });
	combo_target_format_->Bind(wxEVT_CHOICE, &GfxConvDialog::onTargetFormatChanged, this);
	pal_chooser_current_->Bind(wxEVT_CHOICE, [&](wxCommandEvent&) { updatePreviewGfx(); });
	pal_chooser_target_->Bind(wxEVT_CHOICE, [&](wxCommandEvent&) { updatePreviewGfx(); });
	slider_alpha_threshold_->Bind(wxEVT_SLIDER, &GfxConvDialog::onAlphaThresholdChanged, this);
	cb_enable_transparency_->Bind(wxEVT_CHECKBOX, [&](wxCommandEvent&) { updatePreviewGfx(); });
	rb_transparency_colour_->Bind(wxEVT_RADIOBUTTON, [&](wxCommandEvent&) { updatePreviewGfx(); });
	rb_transparency_existing_->Bind(wxEVT_RADIOBUTTON, [&](wxCommandEvent&) { updatePreviewGfx(); });
	rb_transparency_brightness_->Bind(wxEVT_RADIOBUTTON, [&](wxCommandEvent&) { updatePreviewGfx(); });
	colbox_transparent_->Bind(wxEVT_COLOURBOX_CHANGED, [&](wxCommandEvent&) { updatePreviewGfx(); });
...
void GfxConvDialog::onPreviewCurrentMouseDown(wxMouseEvent& e)
{
	...
	ColRGBA col = gfx_current_->image()->pixelAt(imgcoord.x, imgcoord.y, gfx_current_->palette());
