...
TextStylePrefsPanel::TextStylePrefsPanel(wxWindow* parent) : PrefsPanelBase(parent)
{
	...
	ts_current_ = ss_current_.styleFor("default");
	...
	for (unsigned a = 0; a < StyleSet::numSets(); a++)
		choice_styleset_->Append(StyleSet::name(a));
	...
	for (unsigned a = 0; a < ss_current_.nStyles(); a++)
		list_styles_->Append(ss_current_.styleAt(a)->description());
...
void TextStylePrefsPanel::updateStyleControls()
{
	...
	// Get default style
	TextStyle* style_default = ss_current_.styleFor("default");
...
void TextStylePrefsPanel::updatePreview()
{
	...
	{
		txed_override_font      = fp_font_override_->GetSelectedFont().GetFaceName().ToStdString();
...
void TextStylePrefsPanel::onStyleSelected(wxCommandEvent& e)
{
	...
	if (sel == 0)
		...
		ts_current_ = ss_current_.styleFor("selection");
	else
		ts_current_ = ss_current_.styleAt(sel - 2);
...
{
	// Get name for set
	string name = wxGetTextFromUser("Enter Style Set name:", "Save Style Set").ToStdString();
	if (name.empty())
		return;
	...
	for (unsigned a = 0; a < StyleSet::numSets(); a++)
		style_sets.Add(StyleSet::name(a));
