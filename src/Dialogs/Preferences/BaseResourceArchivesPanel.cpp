...
// -----------------------------------------------------------------------------
static bool QueryPathKey(wxRegKey::StdKey hkey, const wxString& path, const wxString& variable, wxString& value)
{
...
void BaseResourceArchivesPanel::autodetect() const
{
	...
	if (doomwaddir.length() || doomwadpath.length())
	{
		...
		for (auto child : list->allChildren())
			iwadnames.Add(WxUtils::stringFromView(child->name()));
...
void BaseResourceArchivesPanel::onBtnAdd(wxCommandEvent& e)
{
	...
	wxFileDialog dialog_open(this,
							 "Choose file(s) to open",
							 dir_last.value.c_str(),
							 wxEmptyString,
							 extensions,
