...
ColourPrefsPanel::ColourPrefsPanel(wxWindow* parent) : PrefsPanelBase(parent)
{
	...
	pg_colours_ = new wxPropertyGrid(this,
									 -1,
									 wxDefaultPosition,
									 wxDefaultSize,
...
void ColourPrefsPanel::refreshPropGrid() const
{
	...
	for (const auto& col : colours)
	{
		...
		auto cdef = ColourConfiguration::definition(col);
		...
		// Add colour
		auto colour = pg_colours_->AppendIn(group, new wxColourProperty(cdef.name, col, WXCOL(cdef.colour)));
	...
	pg_colours_->AppendIn(g_theme,
						  new wxFloatProperty("Line Hilight Width Multiplier",
											  "line_hilight_width",
											  ColourConfiguration::lineHilightWidth()));
	pg_colours_->AppendIn(g_theme,
						  new wxFloatProperty("Line Selection Width Multiplier",
											  "line_selection_width",
											  ColourConfiguration::lineSelectionWidth()));
...
void ColourPrefsPanel::applyPreferences()
{
	...
	{
		// Get colour definition
		auto cdef = ColourConfiguration::definition(colours[a]);
		...
		if (p_alpha && p_add)
		{
			...
			wxVariant v = pg_colours_->GetPropertyValue(cdef_path.c_str());
			wxColour  col;
			col << v; // wut?
...
{
	auto config = choice_configs_->GetStringSelection();
	ColourConfiguration::readConfiguration(config.ToStdString());
	refreshPropGrid();
	MapEditor::forceRefresh(true);
}
