...
Map3DPrefsPanel::Map3DPrefsPanel(wxWindow* parent) : PrefsPanelBase(parent)
{
	...
	cb_max_thing_dist_lock_->Bind(wxEVT_SLIDER, [&](wxCommandEvent&) { updateDistanceControls(); });
	cb_distance_unlimited_->Bind(wxEVT_SLIDER, [&](wxCommandEvent&) { updateDistanceControls(); });
}
