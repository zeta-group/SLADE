...
{
	// Init variables
	col_start_   = ColRGBA::BLACK;
	col_end_     = ColRGBA::WHITE;
	this->steps_ = steps;

	// Minimum height 16
	SetInitialSize({ -1, UI::scalePx(16) });
}
	...
	wxDialog(parent, -1, title)
{
	...
	pal_canvas_original_->allowSelection(PaletteCanvas::SelectionType::Range);
	...
	pal_canvas_target_->allowSelection(PaletteCanvas::SelectionType::Range);
...
void TranslationEditorDialog::openRange(int index)
{
	...
	else if (tr->type() == TransRange::Type::Desaturate)
	{
		// Desaturated colour gradient
	...
	else if (tr->type() == TransRange::Type::Tint)
	{
		...
		label_tint_->SetLabel(fmt::format("{}% ", ttr->amount()));
	}
}
...
void TranslationEditorDialog::onRBPaletteSelected(wxCommandEvent& e)
{
	...
	if (index >= 0)
	{
		...
		// Origin range
		tr->setStart(pal_canvas_original_->getSelectionStart());
		tr->setEnd(pal_canvas_original_->getSelectionEnd());
		// Target range
...
void TranslationEditorDialog::onRBDesaturateSelected(wxCommandEvent& e)
{
	...
	if (index >= 0)
	{
		...
		// Recreate it
		translation_.addRange(TransRange::Type::Desaturate, index);
...
void TranslationEditorDialog::onTintAmountChanged(wxCommandEvent& e)
{
	...
	label_tint_->SetLabel(fmt::format("{}% ", slider_tint_->GetValue()));
}
