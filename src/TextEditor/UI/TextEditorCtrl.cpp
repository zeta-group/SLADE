...
void TextEditorCtrl::setup()
{
	...
	if (txed_show_whitespace > 0)
	{
		...
		SetWhitespaceForeground(true, WXCOL(StyleSet::currentSet()->styleFor("guides")->foreground()));
	}
	else
	...
	CallTipSetForegroundHighlight(WXCOL(StyleSet::currentSet()->styleFor("calltip_hl")->foreground()));
...
void TextEditorCtrl::setupFoldMargin(TextStyle* margin_style)
{
	...
	{
		col_fg = WXCOL(StyleSet::currentSet()->styleFor("foldmargin")->foreground());
		col_bg = WXCOL(StyleSet::currentSet()->styleFor("foldmargin")->background());
	}
...
bool TextEditorCtrl::loadEntry(ArchiveEntry* entry)
{
	...
	if (entry->size() == 0 || !entry->dataRaw())
		return true;

	// Get character entry data
	auto text = wxString::FromUTF8((const char*)entry->dataRaw(), entry->size());
	// If opening as UTF8 failed for some reason, try again as 8-bit data
	if (text.length() == 0)
		text = wxString::From8BitData((const char*)entry->dataRaw(), entry->size());
...
void TextEditorCtrl::getRawText(MemChunk& mc) const
{
	...
	mc.importMem((const uint8_t*)text.data(), text.length());
}
...
void TextEditorCtrl::showCalltip(int position)
{
	...
	call_tip_->setBackgroundColour(ss_current->styleFor("calltip")->background());
	call_tip_->setTextColour(ss_current->styleFor("calltip")->foreground());
	call_tip_->setTextHighlightColour(ss_current->styleFor("calltip_hl")->foreground());
	if (txed_calltips_colourise)
	{
		call_tip_->setFunctionColour(ss_current->styleFor("function")->foreground());
		call_tip_->setTypeColour(ss_current->styleFor("type")->foreground());
		call_tip_->setKeywordColour(ss_current->styleFor("keyword")->foreground());
	}
	if (txed_calltips_use_font)
		call_tip_->setFont(ss_current->defaultFontFace(), ss_current->defaultFontSize() * 0.9);
	else
		call_tip_->setFont("", 0);
...
bool TextEditorCtrl::openCalltip(int pos, int arg, bool dwell)
{
	...
	string word = GetTextRange(WordStartPosition(start, true), WordEndPosition(start, true)).ToStdString();
...
void TextEditorCtrl::updateJumpToList()
{
	...
	jump_to_calculator_ =
		new JumpToCalculator(this, GetText().ToStdString(), language_->jumpBlocks(), language_->jumpBlocksIgnored());
...
{
	wxString space, empty, comment, comment_space;
	...
	{
		auto lineText = GetTextRange(GetLineIndentPosition(line), GetLineEndPosition(line));

		SetTargetStart(GetLineIndentPosition(line));
		SetTargetEnd(GetLineEndPosition(line));

		if (lineText.Find(comment_space) != wxNOT_FOUND)
		{
			if (line == firstLine)
				selectionStartOffs -= comment_space.Len();
			selectionEndOffs -= comment_space.Len();

			lineText.Replace(comment_space, empty, false);
		...
		else if (lineText.Trim(true).Len() != 0)
		{
			...
			ReplaceTarget(lineText.Prepend(comment_space));
		}
	}
	EndUndoAction();
...
{
	wxString space, comment_begin, comment_end;
	space = wxString::FromUTF8(" ");
	if (language_)
	{
	...
	{
		comment_begin = wxString::FromUTF8("/*");
		comment_end   = wxString::FromUTF8("*/");
	}

	size_t comment_begin_len, comment_end_len;
	...
	auto text = GetRange(selection_start, selection_end);

	if (!text.StartsWith(comment_begin, nullptr) && !text.EndsWith(comment_end, nullptr))
	{
		comment_begin = comment_begin.append(space);
		comment_end   = comment_end.Prepend(space);

		ReplaceTarget(text.Prepend(comment_begin).append(comment_end));
		selection_end += (int)(comment_begin.Len() + comment_end.Len());
	}
	else if (text.StartsWith(comment_begin, nullptr) && text.EndsWith(comment_end, nullptr))
	{
		if (text.StartsWith(comment_begin.append(space), nullptr))
			comment_begin_len = comment_begin.Len();
		if (text.EndsWith(comment_end.Prepend(space), nullptr))
			comment_end_len = comment_end.Len();

		ReplaceTarget(text.Remove(0, comment_begin_len).RemoveLast(comment_end_len));
...
void TextEditorCtrl::onKeyDown(wxKeyEvent& e)
{
	...
	for (const auto& name : binds)
	{
		...
		else if (name == "ted_autocomplete")
		{
			...
			string word = GetTextRange(WordStartPosition(GetCurrentPos(), true), GetCurrentPos()).ToStdString();
