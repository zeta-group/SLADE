	...
	: wxChoice(parent, id)
{
	...
	for (int a = 0; a < App::paletteManager()->numPalettes(); a++)
		Append(App::paletteManager()->paletteName(a));
...
// found
// -----------------------------------------------------------------------------
void PaletteChooser::selectPalette(string_view name)
{
	// Go through palettes list
	for (unsigned a = 0; a < GetCount(); a++)
	{
		if (StrUtil::equalCI(WxUtils::stringToView(GetString(a)), name))
		{
			SetSelection(a);
			return;
		}
...
// the list.
// -----------------------------------------------------------------------------
void PaletteChooser::addPalette(string_view name)
{
	// We want it to be just before the "Greyscale" choice
	if (GetCount() > 2)
		Insert(WxUtils::stringFromView(name), GetCount() - 1);
}
