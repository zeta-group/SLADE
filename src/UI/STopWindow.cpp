...
// -----------------------------------------------------------------------------
STopWindow::STopWindow(const wxString& title, string_view id, int x, int y, int width, int height) :
	wxFrame{ nullptr,
			 -1,
			 title,
			 App::platform() == App::Platform::MacOS ? wxDefaultPosition : wxPoint{ x, y },
			 wxSize{ width, height } },
	id_{ id }
{
...
// -----------------------------------------------------------------------------
void STopWindow::enableToolBar(string_view name, bool enable) const
{
...
// [actions]
// -----------------------------------------------------------------------------
void STopWindow::addCustomToolBar(string_view name, const vector<string>& actions) const
{
...
// -----------------------------------------------------------------------------
void STopWindow::removeCustomToolBar(string_view name) const
{
...
void STopWindow::populateToolbarsMenu() const
{
	...
	for (auto a = 0u; a < toolbar_->groups().size(); ++a)
	{
		...
		string name  = StrUtil::replace(group->name(), StrUtil::UNDERSCORE, StrUtil::EMPTY);
