...
VirtualListView::VirtualListView(wxWindow* parent) :
	wxListCtrl(
		...
		wxDefaultPosition,
		wxDefaultSize,
		App::platform() == App::Platform::Windows ? wxLC_REPORT | wxLC_VIRTUAL | wxLC_EDIT_LABELS :
													wxLC_REPORT | wxLC_VIRTUAL)
{
	item_attr_ = std::make_unique<wxListItemAttr>();

	// Set monospace font if configured
	font_normal_ = std::make_unique<wxFont>(wxSystemSettings::GetFont(wxSYS_DEFAULT_GUI_FONT));
	font_monospace_ =
		std::make_unique<wxFont>(WxUtils::getMonospaceFont(wxSystemSettings::GetFont(wxSYS_DEFAULT_GUI_FONT)));
...
// -----------------------------------------------------------------------------
long VirtualListView::focusedItem() const
{
...
bool VirtualListView::defaultSort(long left, long right)
{
	...
	{
		auto ltext = lv_current_->getItemText(left, lv_current_->sort_column_, left);
		auto rtext = lv_current_->getItemText(left, lv_current_->sort_column_, left);
		StrUtil::lowerIP(ltext);
		StrUtil::lowerIP(rtext);
		int result = ltext.compare(rtext);
		if (result == 0)
			return left < right;
		else
...
#ifdef __WXMSW__
	auto   hwnd = ListView_GetHeader((HWND)GetHandle());
	HDITEM header;
	if (hwnd)
	{
...
void VirtualListView::onKeyChar(wxKeyEvent& e)
{
	...
	{
		for (int vlv_char : vlv_chars)
		{
			if (e.GetKeyCode() == vlv_char)
			{
				isRealChar = true;
				break;
			}
