...
protected:
	...
	std::unique_ptr<wxFont>         font_normal_;
	std::unique_ptr<wxFont>         font_monospace_;
	...
	wxString OnGetItemText(long item, long column) const override { return getItemText(item, column, itemIndex(item)); }

	int OnGetItemImage(long item) const override { return getItemIcon(item, 0, itemIndex(item)); }

	int OnGetItemColumnImage(long item, long column) const override
	{
		return getItemIcon(item, column, itemIndex(item));
	}
