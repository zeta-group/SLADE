...
namespace
{
const unsigned NUMROWS = 25;
const unsigned NUMCOLS = 80;
} // namespace
...
void ANSICanvas::drawImage()
{
	...
	auto x = (double)width_;
	auto y = (double)height_;
...
void ANSICanvas::drawCharacter(size_t index)
{
	...
	uint8_t* pic   = picdata_.data() + ((index / NUMCOLS) * width_ * char_height_)
