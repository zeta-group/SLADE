...
void GfxCanvas::drawImage()
{
	...
	auto x = (double)image_.width();
	auto y = (double)image_.height();
	...
	else if (drag_origin_.x < 0) // If not dragging
	{
		...
		if (image_hilight_ && gfx_hilight_mouseover && editing_mode_ == EditMode::View)
		{
	...
	if (editing_mode_ != EditMode::View && cursor_pos_ != POINT_OUTSIDE)
	{
...
{
	// Determine padding
	auto pad = (double)MIN(GetSize().x, GetSize().y) * padding;

	// Get image dimensions
	auto x_dim = (double)image_.width();
	auto y_dim = (double)image_.height();

	// Get max scale for x and y (including padding)
	auto x_scale = ((double)GetSize().x - pad) / x_dim;
	auto y_scale = ((double)GetSize().y - pad) / y_dim;
...
bool GfxCanvas::onImage(int x, int y) const
{
	...
	if (view_type_ == View::Tiled && editing_mode_ == EditMode::View)
		return false;
...
// not on the image
// -----------------------------------------------------------------------------
point2_t GfxCanvas::imageCoords(int x, int y) const
{
...
void GfxCanvas::paintPixel(int x, int y)
{
	...
	else if (editing_mode_ == EditMode::Translate)
	{
...
void GfxCanvas::onMouseLeftDown(wxMouseEvent& e)
{
	...
	{
		// Paint in paint mode
		if (editing_mode_ != EditMode::View)
		{
			drawing_ = true;
			brushCanvas(x, y);
		}
...
void GfxCanvas::onMouseMovement(wxMouseEvent& e)
{
	...
	if (on_image && editing_mode_ != EditMode::View)
	{
