...
bool CTextureCanvas::patchSelected(int index)
{
	...
	return selected_patches_[index] > 0;
}
...
void CTextureCanvas::drawTexture()
{
	...
	if (view_type_ == ViewType::Normal) // No offsets
		glTranslated(texture_->width() * -0.5, texture_->height() * -0.5, 0);
	if (view_type_ != ViewType::Normal) // Sprite offsets
		glTranslated(-texture_->offsetX(), -texture_->offsetY(), 0);
	if (view_type_ == ViewType::Hud) // HUD offsets
	...
	for (size_t a = 0; a < selected_patches_.size(); a++)
	{
		...
		if (selected_patches_[a] == 0)
			continue;

		// Get patch
		CTPatch* patch  = texture_->patch(a);
		auto     epatch = (CTPatchEx*)patch;
	...
	if (hilight_patch_ >= 0 && hilight_patch_ < (int)texture_->nPatches())
	{
		...
		auto       epatch        = (CTPatchEx*)patch;
		GLTexture* patch_texture = patch_textures_[hilight_patch_].get();
...
void CTextureCanvas::drawPatch(int num, bool outside)
{
	...
	if (!patch_textures_[num]->isLoaded())
	{
		...
		if (texture_->loadPatchImage(num, temp, parent_, &palette_))
		{
			...
			patch_textures_[num]->genChequeredTexture(8, ColRGBA::RED, ColRGBA::BLACK);
	}
...
void CTextureCanvas::drawOffsetLines() const
{
	...
	else if (view_type_ == ViewType::Hud)
	{
		glPushMatrix();
		glEnable(GL_LINE_SMOOTH);
		Drawing::drawHud();
...
// -----------------------------------------------------------------------------
point2_t CTextureCanvas::screenToTexPosition(int x, int y) const
{
...
// -----------------------------------------------------------------------------
point2_t CTextureCanvas::texToScreenPosition(int x, int y) const
{
	// Get texture scale
	double tscalex = 1;
	double tscaley = 1;
...
void CTextureCanvas::onMouseEvent(wxMouseEvent& e)
{
	...
	if (e.Moving() || e.Dragging())
	{
		...
		if (e.MiddleIsDown())
		{
			offset_   = offset_ + fpoint2_t(e.GetPosition().x - mouse_prev_.x, e.GetPosition().y - mouse_prev_.y);
			refresh   = true;
			dragging_ = true;
		}
