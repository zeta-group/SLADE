...
public:
	SToolBarSeparator(wxWindow* parent) : wxControl(parent, -1, wxDefaultPosition, wxDefaultSize, wxBORDER_NONE)
	{
		...
		wxControl::SetSizeHints(width, height, width, height);
		wxControl::SetMinSize(wxSize(width, height));
		SetSize(width, height);

		// Set window name
		wxControl::SetName("tb_sep");
...
public:
	...
	{
		// Set size
		wxControl::SetMaxSize(WxUtils::scaledSize(-1, 2));
		wxControl::SetMinSize(WxUtils::scaledSize(-1, 2));

		// Set window name
		wxControl::SetName("tb_vline");
...
// -----------------------------------------------------------------------------
SToolBarGroup::SToolBarGroup(SToolBar* parent, string_view name, bool force_name) :
	wxPanel(parent, -1),
	name_{ name.data(), name.size() }
{
	...
	{
		wxString showname{ name.data(), name.size() };
		if (name[0] == '_')
			showname.erase(0, 1);
		showname += ':';

		auto label = new wxStaticText(this, -1, showname);
...
void SToolBarGroup::hide(bool hide)
{
	...
	if (hide)
		...
		StrUtil::replaceIP(toolbars_hidden.value, hname, StrUtil::EMPTY);
}
...
// -----------------------------------------------------------------------------
SToolBarButton* SToolBarGroup::addActionButton(string_view action, string_view icon, bool show_name)
{
	// Get sizer
...
// -----------------------------------------------------------------------------
void SToolBar::deleteGroup(string_view name)
{
...
void SToolBar::deleteCustomGroups()
{
	...
	for (unsigned a = 0; a < groups_.size(); a++)
	{
		...
		if (!groups_[a]->name().empty() && groups_[a]->name()[0] != '_')
		{
			// Delete group
			delete groups_[a];
			groups_.erase(groups_.begin() + a);
...
// action in [actions]
// -----------------------------------------------------------------------------
void SToolBar::addActionGroup(string_view name, const vector<string>& actions)
{
...
// -----------------------------------------------------------------------------
void SToolBar::enableGroup(string_view name, bool enable)
{
...
void SToolBar::populateGroupsMenu(wxMenu* menu, int start_id)
{
	for (unsigned a = 0; a < groups_.size(); a++)
	{
		...
		StrUtil::replaceIP(name, StrUtil::UNDERSCORE, StrUtil::EMPTY);
...
// -----------------------------------------------------------------------------
SToolBarGroup* SToolBar::group(string_view name)
{
	for (auto group : groups_)
		if (StrUtil::equalCI(group->name(), name))
			return group;
