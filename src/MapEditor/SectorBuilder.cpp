...
// Description: SectorBuilder class - handles sector creation from lines.
//              Traces sector outlines to build an 'edge' list (an edge is
//              basically a side of a line), which is then used to create the
//              appropriate sides etc.
//
...
bool SectorBuilder::traceOutline(MapLine* line, bool front)
{
	...
	for (unsigned a = 0; a < 10000; a++)
	{
		...
		vertex_valid_[edge_next.line->v1Index()] = 0;
		vertex_valid_[edge_next.line->v2Index()] = 0;
...
bool SectorBuilder::pointWithinOutline(double x, double y)
{
	...
	if (nearest >= 0)
	{
		...
		double side = MathStuff::lineSide({ x, y }, o_edges_[nearest].line->seg());
...
SectorBuilder::Edge SectorBuilder::findInnerEdge()
{
	...
	if (!eline)
	{
		...
		vertex_valid_[vertex_right_->index()] = 0;
...
MapSector* SectorBuilder::findCopySector()
{
	...
	for (auto& sector_edge : sector_edges_)
	{
		// Check if the edge's corresponding MapSide has a front sector
		if (sector_edge.line->frontSector())
		{
			// Set sector to copy
			sector_copy = sector_edge.line->frontSector();

			// If the edge is a front edge, use this sector and ignore all else
			if (sector_edge.front)
				break;
		}

		// Check if the edge's corresponding MapSide has a back sector
		if (sector_edge.line->backSector())
		{
			// Set sector to copy
			sector_copy = sector_edge.line->backSector();
...
MapSector* SectorBuilder::findExistingSector(vector<MapSide*>& sides_ignore)
{
	...
	{
		// Check if the edge's corresponding MapSide has a front sector
		if (sector_edge.front && sector_edge.line->frontSector())
		{
			// return sector_edges[a].line->frontSector();
			if (VECTOR_EXISTS(sides_ignore, sector_edge.line->s1()))
				sector = sector_edge.line->frontSector();
			else
				sector_priority = sector_edge.line->frontSector();
		}

		// Check if the edge's corresponding MapSide has a back sector
		if (!sector_edge.front && sector_edge.line->backSector())
		{
			// return sector_edges[a].line->backSector();
			if (VECTOR_EXISTS(sides_ignore, sector_edge.line->s2()))
				sector = sector_edge.line->backSector();
			else
				sector_priority = sector_edge.line->backSector();
		}
	}
...
// -----------------------------------------------------------------------------
// Checks if the traced sector is valid
// (ie. all edges are currently the same (existing) sector)
// -----------------------------------------------------------------------------
...
bool SectorBuilder::traceSector(SLADEMap* map, MapLine* line, bool front)
{
	...
	vertex_valid_.assign(map->nVertices(), 1);
...
void SectorBuilder::createSector(MapSector* sector, MapSector* sector_copy)
{
	...
	for (auto& sector_edge : sector_edges_)
		sector_edge.side_created = map_->setLineSector(sector_edge.line->index(), sector->index(), sector_edge.front);
}
...
void SectorBuilder::drawResult()
{
	...
	for (auto& sector_edge : sector_edges_)
	{
		...
		// Draw line
		glBegin(GL_LINES);
		glVertex2d(sector_edge.line->x1(), sector_edge.line->y1());
		glVertex2d(sector_edge.line->x2(), sector_edge.line->y2());
		glEnd();
	}
}
