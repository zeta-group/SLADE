...
// -----------------------------------------------------------------------------
bool MoveObjects::begin(const fpoint2_t& mouse_pos)
{
	...
	if (context_.editMode() != Mode::Things)
	{
		...
		{
			for (auto& item : items_)
				context_.map().sector(item.index)->getVertices(move_verts);
		}
	}
...
// -----------------------------------------------------------------------------
void MoveObjects::update(const fpoint2_t& mouse_pos)
{
	using MapEditor::Mode;
...
void MoveObjects::end(bool accept)
{
	...
	else if (accept)
	{
		...
		else if (context_.editMode() == Mode::Sectors)
		{
			vector<MapVertex*> sv;
			for (auto& item : items_)
				context_.map().sector(item.index)->getVertices(sv);
