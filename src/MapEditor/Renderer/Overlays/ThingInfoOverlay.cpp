...
void ThingInfoOverlay::draw(int bottom, int right, float alpha)
{
	...
	if (tex)
	{
		...
		if (texwidth > 128.0 || texheight > 128.0)
		{
			double factor = max(texwidth, texheight) / 128.0;
			texwidth /= factor;
			texheight /= factor;
		}
		if (isicon)
		...
		glVertex2d(right - 8 - texwidth, bottom - 8 - texheight);
		glTexCoord2f(0.0f, 1.0f);
		glVertex2d(right - 8 - texwidth, bottom - 8);
		glTexCoord2f(1.0f, 1.0f);
		glVertex2d(right - 8, bottom - 8);
		glTexCoord2f(1.0f, 0.0f);
		glVertex2d(right - 8, bottom - 8 - texheight);
		glEnd();
	}
