...
//              that shows a scrollable list of textures in 3d mode. The
//              currently selected texture is applied in real-time to the
//              currently selected walls / flats, giving a quick 'preview' of
//              how it will look
//
...
// -----------------------------------------------------------------------------
double determineTextureSize(double x, int width)
{
...
// -----------------------------------------------------------------------------
QuickTextureOverlay3d::QuickTextureOverlay3d(MapEditContext* editor) : editor_{ editor }
{
	// Init variables
	...
	if (editor)
	{
		...
		if (sel_type_ > 0)
		{
			...
			for (auto& texinfo : ti)
				tex_names.push_back(texinfo.name);
		}
...
void QuickTextureOverlay3d::applyTexture()
{
	...
	if (!selection.empty())
	{
		for (auto& item : selection)
		{
			...
			// Floor
			else if (item.type == MapEditor::ItemType::Floor && (sel_type_ == 0 || sel_type_ == 2))
			{
				MapSector* sector = editor_->map().sector(item.index);
				if (sector)
					sector->setStringProperty("texturefloor", textures_[current_index_].name);
			}

			// Ceiling
			else if (item.type == MapEditor::ItemType::Ceiling && (sel_type_ == 0 || sel_type_ == 2))
			{
				MapSector* sector = editor_->map().sector(item.index);
				if (sector)
			...
			{
				MapSide* side = editor_->map().side(item.index);
				if (side)
				{
					// Upper
...
void QuickTextureOverlay3d::draw(int width, int height, float fade)
{
	...
	for (unsigned a = 0; a < textures_.size(); a++)
	{
		...
		// Draw texture
		drawTexture(a, x, height - 18, determineTextureSize(x, width), fade);
		x += 136;
