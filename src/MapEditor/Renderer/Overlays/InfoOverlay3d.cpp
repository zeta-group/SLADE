...
//              information about the currently highlighted wall / floor / thing
//              in 3d mode
//
...
void InfoOverlay3D::update(int item_index, MapEditor::ItemType item_type, SLADEMap* map)
{
		...
		|| item_type == MapEditor::ItemType::WallTop)
	{
		...
		if (side == line->s1())
		{
			left_point  = line->v1()->point(MapObject::Point::Mid);
			right_point = line->v2()->point(MapObject::Point::Mid);
			other_side  = line->s2();
		}
		else
		{
			left_point  = line->v2()->point(MapObject::Point::Mid);
			right_point = line->v1()->point(MapObject::Point::Mid);
			other_side  = line->s1();
		}
		...
		if (item_type == MapEditor::ItemType::WallMiddle && other_sector)
		{
			...
			// slopes.
			const auto& floor1   = this_sector->floor().plane;
			const auto& floor2   = other_sector->floor().plane;
			const auto& ceiling1 = this_sector->ceiling().plane;
			const auto& ceiling2 = other_sector->ceiling().plane;
...
void InfoOverlay3D::draw(int bottom, int right, int middle, float alpha)
{
	...
	{
		Drawing::drawText(
			line, middle - (40 * scale) - 4, bottom - y, col_fg, Drawing::Font::Condensed, Drawing::Align::Right);
		y -= line_height;
	}

	// Draw info text lines (right)
	y = height;
	for (const auto& line : info2_)
	{
		Drawing::drawText(line, middle + (40 * scale) + 4, bottom - y, col_fg, Drawing::Font::Condensed);
		y -= line_height;
	}
...
void InfoOverlay3D::drawTexture(float alpha, int x, int y)
{
	...
	if (texname_.size() > 8)
		texname_ = StrUtil::truncate(texname_, 8) + "...";
	Drawing::drawText(
