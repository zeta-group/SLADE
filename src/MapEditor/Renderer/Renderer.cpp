...
// Description: Renderer class - handles rendering/drawing functionality for the
//              map editor
//
...
// -----------------------------------------------------------------------------
// Zooms the view by [amount].
// If [towards_cursor] is true the view will be zoomed towards the current mouse
// cursor position, otherwise towards the center of the screen
// -----------------------------------------------------------------------------
...
double Renderer::interpolateView(bool smooth, double view_speed, double mult)
{
	...
	if (smooth)
	{
		auto mouse_pos = fpoint2_t{ (double)context_.input().mousePos().x, (double)context_.input().mousePos().y };
...
{
	// Determine position
	fpoint3_t pos(thing->position(), 40);
	int       sector = context_.map().sectorAt(thing->position());
	if (sector >= 0)
		pos.z += context_.map().sector(sector)->floor().height;
...
void Renderer::drawSelectionNumbers() const
{
	...
	for (unsigned a = 0; a < selection.size(); a++)
	{
		...
		// Draw text
		Drawing::drawText(fmt::sprintf("%d", a + 1), tp.x, tp.y, col, Drawing::Font::Bold);
	}
	Drawing::setTextOutline(0);
...
// -----------------------------------------------------------------------------
void Renderer::drawLineLength(fpoint2_t p1, fpoint2_t p2, ColRGBA col) const
{
	...
	string length = fmt::sprintf("%d", MathStuff::round(MathStuff::distance(p1, p2)));
...
void Renderer::drawObjectEdit()
{
	...
	if (group.getNearestLine(view_.mapPos(context_.input().mousePos()), 128 / view_.scale(), nl_v1, nl_v2))
	{
		...
		view_.setOverlayCoords(true);
		Drawing::setTextOutline(1.0f, ColRGBA::BLACK);
		Drawing::drawText(fmt::sprintf("%d", length), x, y, ColRGBA::WHITE, Drawing::Font::Bold, Drawing::Align::Center);
		Drawing::setTextOutline(0);
		view_.setOverlayCoords(false);
		glDisable(GL_TEXTURE_2D);
	}
...
void Renderer::draw()
{
	...
	if (context_.editMode() == Mode::Visual)
	{
		...
		if (context_.renderer().renderer3D().itemDistance() >= 0 && camera_3d_show_distance)
		{
			...
			OpenGL::setColour(col);
			Drawing::drawText(
				fmt::sprintf("%d", renderer_3d_.itemDistance()),
				midx + 5,
				midy + 5,
	...
	/*if (map_showfps)
	{
		...
		Drawing::drawText(fmt::sprintf("FPS: %d", afps));
	}*/

	// test
	// Drawing::drawText(fmt::sprintf("Render distance: %1.2f", (double)render_max_dist), 0, 100);
