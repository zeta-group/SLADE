...
void MCAThingSelection::draw()
{
	...
	if (!thing_overlay_square)
	{
		...
		GLTexture* tex;
		if (thing_drawtype == MapRenderer2D::ThingStyle::Round || thing_drawtype == MapRenderer2D::ThingStyle::Sprite)
	...
	glVertex2d(position_.x - r, position_.y - r);
	...
	fade_{ 1.f }
{
	...
	for (auto& line : lines)
	{
		...
		// Add line
		this->lines_.emplace_back(line->x1(), line->y1(), line->x2(), line->y2());

		// Calculate line direction tab
		fpoint2_t mid = line->point(MapObject::Point::Mid);
		fpoint2_t tab = line->dirTabPoint();

		this->tabs_.emplace_back(mid.x, mid.y, tab.x, tab.y);
	}
}
