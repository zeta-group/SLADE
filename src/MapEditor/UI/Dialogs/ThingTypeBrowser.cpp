...
bool ThingBrowserItem::loadImage()
{
	...
	if (!tex && use_zeth_icons && type_.zethIcon() >= 0)
	{
		...
		tex = MapEditor::textureManager().editorImage(fmt::sprintf("zethicons/zeth%02d", type_.zethIcon()));
	}
	if (!tex)
	{
		// Sprite not found, try an icon
		tex = MapEditor::textureManager().editorImage(fmt::sprintf("thing/%s", type_.icon()));
	}
	if (!tex)
	{
...
{
	// Set window title
	wxDialog::SetTitle("Browse Thing Types");
	...
	// Set browser options
	canvas_->setItemNameType(BrowserItem::NameType::Index);
	setupViewOptions();
...
void ThingTypeBrowser::setupViewOptions()
{
	if (browser_thing_tiles)
	{
		...
		setItemSize(48);
		setItemViewType(BrowserItem::ViewType::Tiles);
	}
	else
	{
		setFont(Drawing::Font::Bold);
		setItemSize(80);
		setItemViewType(BrowserItem::ViewType::Normal);
	}
...
int ThingTypeBrowser::getSelectedType()
{
	...
	if (selected)
	{
		Log::info(fmt::sprintf("Selected item %d", selected->index()));
