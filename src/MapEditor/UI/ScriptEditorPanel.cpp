...
{
	// Init variables
	entry_script_   = std::make_unique<ArchiveEntry>();
	entry_compiled_ = std::make_unique<ArchiveEntry>();
	...
	if (name == "mapw_script_compile")
	{
		...
		if (lang == "acs_hexen")
			EntryOperations::compileACS(
				entry_script_.get(), true, entry_compiled_.get(), (wxFrame*)MapEditor::windowWx());
		else if (lang == "acs_zdoom")
			EntryOperations::compileACS(
				entry_script_.get(), false, entry_compiled_.get(), (wxFrame*)MapEditor::windowWx());
	}
...
void ScriptEditorPanel::onWordListActivate(wxCommandEvent& e)
{
	...
	auto           word_wx = list_words_->GetItemText(item);
	auto           word    = word_wx.ToStdString();
	...
	if (text_editor_->GetSelectionStart() < text_editor_->GetSelectionEnd())
	{
		...
		text_editor_->ReplaceSelection(word_wx);
