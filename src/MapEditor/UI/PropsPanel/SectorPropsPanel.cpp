...
// -----------------------------------------------------------------------------
void FlatTexCanvas::setTexture(string_view tex)
{
...
void SectorPropsPanel::applyChanges()
{
	for (auto& object : objects_)
	{
		...
		if (!fcb_floor_->GetValue().IsEmpty())
			sector->setStringProperty("texturefloor", fcb_floor_->GetValue().ToStdString());

		// Ceiling texture
		if (!fcb_ceiling_->GetValue().IsEmpty())
			sector->setStringProperty("textureceiling", fcb_ceiling_->GetValue().ToStdString());
		...
		if (!text_light_->GetValue().IsEmpty())
			sector->setIntProperty("lightlevel", text_light_->getNumber(sector->light()));

		// Tag
		if (!text_tag_->GetValue().IsEmpty())
			sector->setIntProperty("id", text_tag_->getNumber(sector->id()));
	}
...
void SectorPropsPanel::onTextureChanged(wxCommandEvent& e)
{
	...
	if (e.GetEventObject() == fcb_floor_)
		gfx_floor_->setTexture(fcb_floor_->GetValue().ToStdString());

	// Ceiling
	else if (e.GetEventObject() == fcb_ceiling_)
		gfx_ceiling_->setTexture(fcb_ceiling_->GetValue().ToStdString());
