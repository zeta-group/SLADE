...
// -----------------------------------------------------------------------------
// Adds the UDMF property [prop] to the grid, under [basegroup].
// Will add the correct property cell type for the UDMF property
// -----------------------------------------------------------------------------
	...
	wxPropertyGrid* grid)
{
	...
	wxString groupname;
	if (!basegroup.empty())
		groupname.append(basegroup.data(), basegroup.size()).append(1, '.');
	...
	wxString propname;
	if (!basegroup.empty())
		propname.append(basegroup.data(), basegroup.size()).append(1, '.');
	...
	{
		MOPGTagProperty::TagType tagtype;
		if (objtype == MapObject::Type::Line)
			tagtype = MOPGTagProperty::TagType::Line;
		else if (objtype == MapObject::Type::Thing)
			tagtype = MOPGTagProperty::TagType::Thing;
		else
			tagtype = MOPGTagProperty::TagType::Sector;
...
void MapObjectPropsPanel::setupType(MapObject::Type objtype)
{
	...
	else if (objtype == MapObject::Type::Line)
	{
		...
		if (!propHidden("special"))
		{
			...
			{
				auto prop_tag = new MOPGTagProperty(MOPGTagProperty::TagType::Sector, "Sector Tag", "arg0");
	...
	else if (objtype == MapObject::Type::Sector)
	{
		...
		if (!propHidden("id"))
		{
			auto prop_tag = new MOPGTagProperty(MOPGTagProperty::TagType::Sector, "Tag/ID", "id");
			prop_tag->setParent(this);
			properties_.push_back(prop_tag);
	...
	else if (objtype == MapObject::Type::Thing)
	{
		...
		{
			auto prop_id = new MOPGTagProperty(MOPGTagProperty::TagType::Thing, "ID", "id");
			prop_id->setParent(this);
			properties_.push_back(prop_id);
...
void MapObjectPropsPanel::openObjects(vector<MapObject*>& objects)
{
	...
	if (MapEditor::editContext().mapDesc().format == MAP_UDMF)
	{
		for (auto& object : objects)
		{
			...
			for (auto& objprop : object->props().allProperties())
			{
				// Ignore unset properties
				if (!objprop.value.hasValue())
					continue;

				// Ignore side property
				if (StrUtil::startsWith(objprop.name, "side1.") || StrUtil::startsWith(objprop.name, "side2."))
					continue;

				// Check if hidden
				if (VECTOR_EXISTS(hide_props_, objprop.name))
					continue;
				...
				{
					if (property->propName() == objprop.name)
					{
						exists = true;
						break;
					}
				...
				if (!exists)
				{
					...
					{
					case Property::Type::Bool: addBoolProperty(group_custom_, objprop.name, objprop.name); break;
					case Property::Type::Int: addIntProperty(group_custom_, objprop.name, objprop.name); break;
					case Property::Type::Float: addFloatProperty(group_custom_, objprop.name, objprop.name); break;
					default: addStringProperty(group_custom_, objprop.name, objprop.name); break;
					}
				}
			}
		}
...
void MapObjectPropsPanel::onBtnAdd(wxCommandEvent& e)
{
	...
	if (dlg.ShowModal() == wxID_OK)
	{
		...
		// Get name entered
		string propname = text_name->GetValue().ToStdString();
		StrUtil::lowerIP(propname);
		if (propname.empty() || StrUtil::contains(propname, ' ')) // TODO: Proper regex check
		{
