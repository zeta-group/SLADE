...
{
public:
	SIFDoomGfx(string_view id = "doom", string_view name = "Doom Gfx", uint8_t reliability = 230) :
...
protected:
	static bool readDoomFormat(SImage& image, MemChunk& data, int version)
	{
		// Init variables
		...
		for (int c = 0; c < width; c++)
		{
			...
			uint32_t col_offset = col_offsets[c];
	...
	bool writeImage(SImage& image, MemChunk& out, Palette* pal, int index) override
	{
		...
		patch_header_t header{
			(short)image.offset().y, (short)image.offset().x, (short)image.width(), (short)image.height()
		};
		...
		for (size_t c = 0; c < columns.size(); c++)
		{
			...
			for (auto& post : columns[c].posts)
			{
				...
				// Write pixels
				for (unsigned char& pixel : post.pixels)
					out.write(&pixel, 1);
...
public:
	...
	SImage::Info getInfo(MemChunk& mc, int index) override
	{
		auto info   = SIFDoomGfx::getInfo(mc, index);
		info.format = id_;
		return info;
	}
