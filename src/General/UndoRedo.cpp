...
// -----------------------------------------------------------------------------
UndoLevel::UndoLevel(string_view name) : name_{ name.data(), name.size() }, timestamp_{ wxDateTime::Now() } {}
...
// recorded
// -----------------------------------------------------------------------------
string UndoLevel::getTimeStamp(bool date, bool time) const
{
	if (date && !time)
...
// -----------------------------------------------------------------------------
bool UndoLevel::readFile(string_view filename)
{
	return true;
}
...
// -----------------------------------------------------------------------------
bool UndoLevel::writeFile(string_view filename)
{
	return true;
}
...
void UndoLevel::createMerged(vector<UndoLevel*>& levels)
{
	...
	{
		for (unsigned b = 0; b < level->undo_steps_.size(); b++)
			undo_steps_.push_back(level->undo_steps_[b]);
...
string UndoManager::redo()
{
	...
	if (current_level_index_ == undo_levels_.size() - 1 || undo_levels_.empty())
		return "";
