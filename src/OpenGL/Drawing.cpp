...
private:
	...
	sf::Font font_condensed_;
	sf::Font font_bold_;
	sf::Font font_boldcondensed_;
	sf::Font font_mono_;
	sf::Font font_small_;
#else
	FTFont* font_normal;
	FTFont* font_condensed_;
	FTFont* font_bold;
...
int FontManager::initFonts()
{
	...
	if (entry)
		++ret, font_normal_.loadFromMemory((const char*)entry->dataRaw(), entry->size());

	// Condensed
	entry = App::archiveManager().programResourceArchive()->entryAtPath("fonts/dejavu_sans_c.ttf");
	if (entry)
		++ret, font_condensed_.loadFromMemory((const char*)entry->dataRaw(), entry->size());

	// Bold
	entry = App::archiveManager().programResourceArchive()->entryAtPath("fonts/dejavu_sans_b.ttf");
	if (entry)
		++ret, font_bold_.loadFromMemory((const char*)entry->dataRaw(), entry->size());

	// Condensed Bold
	entry = App::archiveManager().programResourceArchive()->entryAtPath("fonts/dejavu_sans_cb.ttf");
	if (entry)
		++ret, font_boldcondensed_.loadFromMemory((const char*)entry->dataRaw(), entry->size());

	// Monospace
	entry = App::archiveManager().programResourceArchive()->entryAtPath("fonts/dejavu_mono.ttf");
	if (entry)
		++ret, font_small_.loadFromMemory((const char*)entry->dataRaw(), entry->size());
...
int FontManager::initFonts()
{
	...
	if (font_condensed_)
	{
		delete font_condensed_;
		font_condensed_ = NULL;
	}
	if (font_bold)
	{
		delete font_bold;
	...
	if (entry)
	{
		font_condensed_ = new FTTextureFont(entry->getData(), entry->getSize());
		font_condensed_->FaceSize(UI::scalePx(gl_font_size));

		// Check it loaded ok
		if (Font::Condensed->Error())
		{
			delete Font::Condensed;
			Font::Condensed = NULL;
		}
...
sf::Font* FontManager::getFont(Drawing::Font font)
{
	switch (font)
	{
	case Drawing::Font::Normal: return &font_normal_;
	case Drawing::Font::Condensed: return &font_condensed_;
	case Drawing::Font::Bold: return &font_bold_;
	case Drawing::Font::BoldCondensed: return &font_boldcondensed_;
	case Drawing::Font::Monospace: return &font_mono_;
	case Drawing::Font::Small: return &font_small_;
	default: return &font_normal_;
	};
}
...
FTFont* FontManager::getFont(int font)
{
	...
	case Drawing::Font::Condensed: return Font::Condensed;
...
// -----------------------------------------------------------------------------
void Drawing::drawLine(const fpoint2_t& start, const fpoint2_t& end)
{
	glBegin(GL_LINES);
...
// Draws a line from [start] to [end]
// -----------------------------------------------------------------------------
void Drawing::drawLineTabbed(const fpoint2_t& start, const fpoint2_t& end, double tab, double tab_max)
{
	// Draw line
	glBegin(GL_LINES);
	glVertex2d(start.x, start.y);
...
// -----------------------------------------------------------------------------
void Drawing::drawRect(const fpoint2_t& tl, const fpoint2_t& br)
{
...
// -----------------------------------------------------------------------------
void Drawing::drawFilledRect(const fpoint2_t& tl, const fpoint2_t& br)
{
	glBegin(GL_QUADS);
	glVertex2d(tl.x, tl.y);
	glVertex2d(tl.x, br.y);
...
// -----------------------------------------------------------------------------
void Drawing::drawEllipse(const fpoint2_t& mid, double radius_x, double radius_y, int sides, const ColRGBA& colour)
{
	// Set colour
	OpenGL::setColour(colour, false);
...
// -----------------------------------------------------------------------------
void Drawing::drawFilledEllipse(const fpoint2_t& mid, double radius_x, double radius_y, int sides, const ColRGBA& colour)
{
	// Set colour
	OpenGL::setColour(colour, false);
	...
	double     max_scale)
{
	...
	return { x1 + width * 0.5 - (scale * tex->getWidth() * 0.5),
...
// -----------------------------------------------------------------------------
fpoint2_t Drawing::textExtents(const string& text, Font font)
{
	// Setup SFML string
	sf::Text sf_str;
	sf_str.setString(text);
...
// the rendered text string are written to it.
// -----------------------------------------------------------------------------
void Drawing::drawText(string text, int x, int y, ColRGBA colour, int font, int alignment, frect_t* bounds)
{
	// Get desired font
...
void TextBox::split(string_view text)
{
	...
	while (line < lines_.size())
	{
		...
		// Split line
		string nl   = lines_[line].substr(sc);
...
void TextBox::draw(int x, int y, const ColRGBA& colour, Drawing::Align alignment)
{
	...
	{
		Drawing::drawText(line, x, y, colour, font_, alignment, &b);
...
CONSOLE_COMMAND(d_testfont, 1) {
	...
	if (entry) {
		if (Drawing::Font::Condensed) {
			delete Drawing::Font::Condensed;
			Drawing::Font::Condensed = NULL;
		}
		...
		Drawing::Font::Condensed = new FTTextureFont(entry->getData(), entry->getSize());
		Drawing::Font::Condensed->FaceSize(size);
	}
}
*/
