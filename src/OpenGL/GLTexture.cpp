...
bool GLTexture::loadData(const uint8_t* data, uint32_t width, uint32_t height, bool add)
{
	...
	height_ = height;
	scale_  = { 1., 1. };
	parts_.push_back(ntex);

	return true;
}

// -----------------------------------------------------------------------------
// Loads raw RGBA data to the texture.
...
// Only used internally, the portion must be 128x128 in size
// -----------------------------------------------------------------------------
bool GLTexture::loadImagePortion(SImage* image, const rect_t& rect, Palette* pal, bool add)
{
	// Check image was given
	if (!image)
		return false;
...
// -----------------------------------------------------------------------------
bool GLTexture::genChequeredTexture(uint8_t block_size, const ColRGBA& col1, const ColRGBA& col2)
{
...
bool GLTexture::bind()
{
	...
	if (!loaded_ || parts_.empty())
		return false;

	// Bind the texture
	glBindTexture(GL_TEXTURE_2D, parts_[0].id);
...
bool GLTexture::draw2d(double x, double y, bool flipx, bool flipy)
{
	...
	else
	{
		...
		while (top < height_ && top >= 0)
		{
			...
			{
				if (tex_index < parts_.size())
				{
					// Bind the texture
					glBindTexture(GL_TEXTURE_2D, parts_[tex_index].id);
...
// -----------------------------------------------------------------------------
ColRGBA GLTexture::averageColour(rect_t area)
{
	// Check texture is loaded
	if (!loaded_ || parts_.empty())
	...
	// Get texture pixels
	vector<uint8_t> pixels(width_ * height_ * 8);
	glBindTexture(GL_TEXTURE_2D, parts_[0].id);
