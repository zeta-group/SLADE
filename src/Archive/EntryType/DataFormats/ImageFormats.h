	...
	int isThisFormat(MemChunk& mc) override
	{
		...
		if (mc.size() > 30)
		{
			...
			if (mc[0] == 'B' && mc[1] == 'M')
			{
				...
				if (READ_L32(mc, 2) == (int)mc.size() && READ_L32(mc, 6) == 0)
					return EDF_TRUE;
...
public:
	...
	int isThisFormat(MemChunk& mc) override
	{
		...
		for (size_t i = 74; i < 128; ++i)
			if (mc[i] != 0)
				return EDF_FALSE;
		int16_t offsx = (int16_t)READ_L16(mc, 4);
		int16_t limx  = (int16_t)READ_L16(mc, 8);
...
public:
	...
	int isThisFormat(MemChunk& mc) override
	{
		...
		READ_B16(mc, 4); // x offset
		READ_B16(mc, 6); // y offset
...
public:
	...
	int isThisFormat(MemChunk& mc) override
	{
		...
		if (size < 4)
			return EDF_FALSE;
		if ((4 + (READ_L16(mc, 0) * READ_L16(mc, 2))) != (int)mc.size())
			return EDF_FALSE;
