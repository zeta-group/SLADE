	...
	int isThisFormat(MemChunk& mc) override
	{
		...
		if (mc.size() > 36)
		{
			...
			if (mc[0] == 'R' && mc[1] == 'I' && mc[2] == 'F' && mc[3] == 'F' && mc[8] == 'R' && mc[9] == 'M'
				...
				&& (READ_L32(mc, 4) + 8 == (int)mc.size()) && (READ_L32(mc, 16) < READ_L32(mc, 4)))
				return EDF_TRUE;
		}
...
public:
	...
	int isThisFormat(MemChunk& mc) override
	{
		...
		if (mc.size() > 4)
		{
			...
			// checked easily.
			if ((int)mc.size() == 4 + READ_L16(mc, 2))
				return EDF_TRUE;
		}
		return EDF_FALSE;
	}
...
public:
	...
	int isThisFormat(MemChunk& mc) override
	{
		int fmt = Audio::detectRiffWavFormat(mc);
		if (fmt == Audio::WavFormat::Unknown || fmt == Audio::WavFormat::MP3)
			return EDF_FALSE;
		if (fmt <= Audio::WavFormat::MULAW || fmt == Audio::WavFormat::XTNSBL)
			return EDF_TRUE;
		return EDF_MAYBE;
	}
};
...
public:
	...
	int isThisFormat(MemChunk& mc) override
	{
		...
		int rwfmt = Audio::detectRiffWavFormat(mc);
		if (rwfmt == Audio::WavFormat::MP3)
			return EDF_TRUE;
...
public:
	...
	int isThisFormat(MemChunk& mc) override { return (mc.size() > 0 ? EDF_MAYBE : EDF_FALSE); }
};
