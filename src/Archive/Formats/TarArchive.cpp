...
bool TarArchive::write(MemChunk& mc, bool update)
{
	...
	for (size_t a = 0; a < listsize; ++a)
	{
		...
		if (entries[a]->type() == EntryType::folderType())
		{
			...
			writeOctal(makeChecksum(&header), header.chksum, 7);
			mc.write(&header, 512);
		...
		else
		{
			...
			writeOctal(entries[a]->size(), header.size, 12);
			writeOctal(makeChecksum(&header), header.chksum, 7);
			size_t padsize = entries[a]->size() % 512;
			if (padsize)
				padsize = 512 - padsize;
			mc.write(&header, 512);
			mc.write(entries[a]->dataRaw(), entries[a]->size());
			if (padsize)
...
// octal number in ASCII. Returns -1 if not a number.
// -----------------------------------------------------------------------------
int TarArchive::tarSum(const char* field, int size)
{
...
// -----------------------------------------------------------------------------
bool TarArchive::writeOctal(size_t sum, char* field, int size)
{
...
// -----------------------------------------------------------------------------
bool TarArchive::checksum(TarHeader* header)
{
...
// -----------------------------------------------------------------------------
size_t TarArchive::makeChecksum(TarHeader* header)
{
	size_t   checksum = 0;
...
// -----------------------------------------------------------------------------
void TarArchive::defaultHeader(TarHeader* header)
{
	if (header == nullptr)
		return;

	memset(header->name, 0, 100);                      // Name: fill with zeroes
	writeOctal(0777, header->mode, 8);                 // Mode: free for all
	writeOctal(1, header->uid, 8);                     // UID: first non-root user
	writeOctal(1, header->gid, 8);                     // GID: first non-root group
	writeOctal(0, header->size, 12);                   // File size: 0 for now
	writeOctal(::wxGetLocalTime(), header->mtime, 12); // mtime: now
