...
bool isNamespaceEntry(ArchiveEntry* entry)
{
	...
	return StrUtil::endsWith(uname, "_START") || StrUtil::endsWith(uname, "_END");
}
} // namespace
...
// -----------------------------------------------------------------------------
// Returns true if the archive can be written
// -----------------------------------------------------------------------------
...
void WadArchive::updateNamespaces()
{
	...
	for (unsigned a = 0; a < numEntries(); a++)
	{
		...
		auto          u_name = entry->upperName();

		// Check for namespace begin
		if (StrUtil::endsWith(u_name, "_START"))
		{
	...
	if (numEntries() > 2090 && rootDir()->entryAt(0)->name() == "WALLSTRT"
		&& rootDir()->entryAt(numEntries() - 2)->name() == "TABLES")
	{
...
bool WadArchive::hasFlatHack()
{
	for (auto& ns : namespaces_)
	{
		if (ns.name == "f")
		{
			return (ns.start_index == 0 && ns.start->size() != 0);
		}
	}
	return false;
}
...
bool WadArchive::write(string_view filename, bool update)
{
	...
	file.write<uint32_t>(num_lumps);
	file.write<uint32_t>(dir_offset);
	...
	for (uint32_t l = 0; l < num_lumps; l++)
	{
		...
		if (entry->size())
		{
			file.write((char*)entry->dataRaw(), entry->size());
		}
	}
	...
	for (uint32_t l = 0; l < num_lumps; l++)
	{
		entry            = getEntry(l);
		char     name[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
		uint32_t offset  = getEntryOffset(entry);
		uint32_t size    = entry->size();

		for (size_t c = 0; c < entry->name().length() && c < 8; c++)
			name[c] = entry->name()[c];

		file.write<uint32_t>(offset);
...
ArchiveEntry* WadArchive::addEntry(ArchiveEntry* entry, string_view add_namespace, bool copy)
{
	...
	for (auto& special_namespace : special_namespaces)
	{
		if (add_namespace == special_namespace.name)
		{
			addNewEntry(special_namespace.letter + "_start");
			addNewEntry(special_namespace.letter + "_end");
...
bool WadArchive::removeEntry(ArchiveEntry* entry)
{
	...
	{
		// Update namespaces if necessary
		if (StrUtil::endsWith(name, "_START") || StrUtil::endsWith(name, "_END"))
			updateNamespaces();
...
// -----------------------------------------------------------------------------
// Returns the mapdesc_t information about the map beginning at [maphead].
// If [maphead] is not really a map header entry, an invalid mapdesc_t will be
// returned (mapdesc_t::head == NULL)
// -----------------------------------------------------------------------------
Archive::MapDesc WadArchive::getMapInfo(ArchiveEntry* maphead)
{
	...
	if (StrUtil::equalCI(maphead->nextEntry()->name(), "TEXTMAP"))
	{
		// Get map info
		map.head   = maphead;
		map.format = MAP_UDMF;
		...
		while (true)
		{
			if (!entry || StrUtil::equalCI(entry->name(), "ENDMAP"))
				break;

			// Check for unknown map lumps
			bool known = false;
			for (auto& map_lump : map_lumps)
			{
				if (StrUtil::equalCI(entry->name(), map_lump))
				{
					known = true;
					break;
				}
	...
	while (entry)
	{
		...
		{
			if (StrUtil::equalCI(entry->name(), map_lumps[a]))
			{
				mapentry              = true;
			...
			{
				string name = maphead->nameNoExt().to_string();
				name.insert(0, "GL_");
				if (StrUtil::equalCI(entry->name(), name))
				{
					mapentry              = true;
					existing_map_lumps[a] = 1;
					break;
...
vector<Archive::MapDesc> WadArchive::detectMaps()
{
	...
	for (auto& map : maps)
	{
		...
		ArchiveEntry* head = map.head;
		while (head && head != map.end->nextEntry())
		{
			head->exProp("MapFormat") = format;
			head                      = head->nextEntry();
		}
	}
...
ArchiveEntry* WadArchive::findFirst(SearchOptions& options)
{
	...
	while (entry != end)
	{
		...
		{
			if (!StrUtil::matchesCI(entry->name(), options.match_name))
			{
				entry = entry->nextEntry();
				continue;
			}
...
// -----------------------------------------------------------------------------
bool WadArchive::isWadArchive(string_view filename)
{
