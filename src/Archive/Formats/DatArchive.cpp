...
bool DatArchive::open(MemChunk& mc)
{
	...
	for (uint32_t d = 0; d < num_lumps; d++)
	{
		...
		{
			Log::info(fmt::format("DatArchive::open: Dat archive is invalid or corrupt at entry {}", d));
...
// -----------------------------------------------------------------------------
// Returns the namespace that the entry at [index] is in
// -----------------------------------------------------------------------------
...
ArchiveEntry* DatArchive::addEntry(ArchiveEntry* entry, unsigned position, ArchiveTreeNode* dir, bool copy)
{
	...
	if (StrUtil::matchesCI(entry->name(), "START*") || StrUtil::matchesCI(entry->name(), "END*"))
		updateNamespaces();
...
// nullptr if the entry is invalid
// -----------------------------------------------------------------------------
...
bool DatArchive::removeEntry(ArchiveEntry* entry)
{
	...
	{
		// Update namespaces if necessary
		if (StrUtil::matchesCI(name, "START*") || StrUtil::matchesCI(name, "END*"))
			updateNamespaces();
...
bool DatArchive::swapEntries(ArchiveEntry* entry1, ArchiveEntry* entry2)
{
	...
	if (ok)
	{
		...
		if (StrUtil::matchesCI(entry1->name(), "START*") || StrUtil::matchesCI(entry1->name(), "END*")
			|| StrUtil::matchesCI(entry2->name(), "START*") || StrUtil::matchesCI(entry2->name(), "END*"))
			updateNamespaces();
...
bool DatArchive::loadEntryData(ArchiveEntry* entry)
{
	...
	if (!file.IsOpened())
	{
		Log::info(fmt::format("DatArchive::loadEntryData: Failed to open datfile {}", filename_));
		return false;
	}
