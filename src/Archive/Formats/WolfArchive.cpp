...
// filename.
// On Windws systems, filenames are case-insensitive, so the source filename is
// returned as-is.
// On other systems, we instead take only the path (assumed to be correct, since
// we got it from attempting to open a file that does exist) and then we iterate
...
// -----------------------------------------------------------------------------
// Adds height and width information to a picture.
// Needed because Wolfenstein 3-D is just that much of a horrible hacky mess.
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Needed to read VGAGRAPH content. Adapted from Wolf3D code, but with dead code
// removed from it.
// -----------------------------------------------------------------------------
struct huffnode
{
...
bool WolfArchive::open(string_view filename)
{
	...
	else
	{
		...
		MemChunk mc;
		if (!mc.importFile(filename.to_string()))
		{
...
bool WolfArchive::openAudio(MemChunk& head, MemChunk& data)
{
	...
	// Read the offsets
	UI::setSplashProgressMessage("Reading Wolf archive data");
	const auto* offsets = (const uint32_t*)head.data();
	MemChunk    edata;
...
bool WolfArchive::openMaps(MemChunk& head, MemChunk& data)
{
	...
	// Read the offsets
	UI::setSplashProgressMessage("Reading Wolf archive data");
	const auto* offsets = (const uint32_t*)(2 + head.data());
	for (uint32_t d = 0; d < num_lumps; d++)
	{
		// Update splash window progress
		UI::setSplashProgress(((float)d / (float)num_lumps));
...
bool WolfArchive::openGraph(MemChunk& head, MemChunk& data, MemChunk& dict)
{
	...
	for (uint32_t d = 0; d < num_lumps; d++)
	{
		...
		uint32_t size = READ_L24(head, ((d + 1) * 3)) - offset;
	...
	for (size_t a = 0; a < numEntries(); a++)
	{
		...
		if (a == 0)
			pictable = (uint16_t*)entry->dataRaw();
		else if (pictable && a >= WC(STARTPICS) && a < WC(STARTPICM))
		{
...
// -----------------------------------------------------------------------------
//
// WolfArchive Class Static Functions
//
// -----------------------------------------------------------------------------
