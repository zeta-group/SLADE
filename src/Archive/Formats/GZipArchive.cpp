...
bool GZipArchive::open(MemChunk& mc)
{
	...
	bool fname = (header[3] & GZIP_FLG_FNAME) != 0;
	bool fcmnt = (header[3] & GZIP_FLG_FCMNT) != 0;
	flags_     = header[3];
...
bool GZipArchive::write(MemChunk& mc, bool update)
{
	...
	{
		MemChunk stream;
		if (Compression::GZipDeflate(getEntry(0)->data(), stream, 9))
		{
			...
			// Write mtime
			uint32_t working = wxUINT32_SWAP_ON_BE(mtime_);
			mc.write(&working, 4);
			...
			{
				mc.write(getEntry(0)->name().data(), getEntry(0)->name().size());
				uint8_t zero = 0;
			...
			{
				mc.write(comment_.c_str(), comment_.size());
				uint8_t zero = 0;
				mc.write(&zero, 1); // Terminate string
			}
...
// -----------------------------------------------------------------------------
//
// GZipArchive Class Static Functions
//
// -----------------------------------------------------------------------------
