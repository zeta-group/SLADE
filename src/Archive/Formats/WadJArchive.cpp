...
bool WadJArchive::open(MemChunk& mc)
{
	...
	{
		Log::error(fmt::format("WadJArchive::openFile: File {} has invalid header", filename_));
	...
	for (uint32_t d = 0; d < num_lumps; d++)
	{
		...
		{
			Log::error(1, "WadJArchive::open: Wad archive is invalid or corrupt");
			Global::error =
		...
		// Create & setup lump
		ArchiveEntry* nlump = new ArchiveEntry(name, actualsize);
...
string WadJArchive::detectNamespace(size_t index, ArchiveTreeNode* dir)
{
	...
	{
		auto nextentry = getEntry(index + 1);
		if (nextentry && nextentry->name() == StrUtil::FULLSTOP)
			return "sprites";
	}
...
// -----------------------------------------------------------------------------
//
// WadJArchive Class Static Functions
//
// -----------------------------------------------------------------------------
