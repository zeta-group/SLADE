...
bool ChasmBinArchive::open(MemChunk& mc)
{
	...
	{
		Log::error(1, "ChasmBinArchive::open: Opening failed, invalid header");
	...
	for (uint16_t i = 0; i < num_entries; ++i)
	{
		...
		{
			Log::error(1, "ChasmBinArchive::open: Bin archive is invalid or corrupt (entry goes past end of file)");
...
bool ChasmBinArchive::write(MemChunk& mc, bool update)
{
	...
	for (uint16_t i = 0; i < num_entries; ++i)
	{
		...
		{
			Log::warning(fmt::format("Entry {} name is too long, it will be truncated", name));
