...
bool ChasmBinArchive::open(MemChunk& mc)
{
	...
	{
		Log::info(1, "ChasmBinArchive::open: Opening failed, invalid header");
	...
	for (uint16_t i = 0; i < num_entries; ++i)
	{
		...
		{
			Log::info(1, "ChasmBinArchive::open: Bin archive is invalid or corrupt (entry goes past end of file)");
...
bool ChasmBinArchive::write(MemChunk& mc, bool update)
{
	...
	{
		Log::info(
			fmt::format("ChasmBinArchive::write: Bin archive can contain no more than {} entries", MAX_ENTRY_COUNT));
	...
	for (uint16_t i = 0; i < num_entries; ++i)
	{
		...
		{
			Log::info(fmt::format("Warning: Entry {} name is too long, it will be truncated", name));
...
bool ChasmBinArchive::loadEntryData(ArchiveEntry* entry)
{
	...
	if (!file.IsOpened())
	{
		Log::info(fmt::format("ChasmBinArchive::loadEntryData: Unable to open archive file {}", filename_));
		return false;
	}
