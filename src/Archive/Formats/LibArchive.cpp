...
bool LibArchive::open(MemChunk& mc)
{
	...
	for (uint32_t d = 0; d < num_lumps; d++)
	{
		...
		{
			Log::info(1, "LibArchive::open: Lib archive is invalid or corrupt");
...
bool LibArchive::loadEntryData(ArchiveEntry* entry)
{
	...
	if (!file.IsOpened())
	{
		Log::info(fmt::format("LibArchive::loadEntryData: Failed to open libfile {}", filename_));
		return false;
	}
...
// -----------------------------------------------------------------------------
ArchiveEntry* LibArchive::addEntry(ArchiveEntry* entry, string_view add_namespace, bool copy)
{
...
// -----------------------------------------------------------------------------
//
// LibArchive Class Static Functions
//
// -----------------------------------------------------------------------------
