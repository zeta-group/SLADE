...
void parseStates(Tokenizer& tz, PropertyList& props)
{
	...
	while (!tz.atEnd())
	{
		...
		if (tz.checkNext(":"))
		{
			...
			states.push_back(StrUtil::lower(tz.current().text));
	...
	Log::debug(2, fmt::format("Parsed states, got sprite {}", props["sprite"].stringValue()));
	...
	//		{
	//			if (StrUtil::equalCI(myspritestate, "spawn"))
	//				mypriority = StateSprites::Spawn;
	//			else if (StrUtil::equalCI(myspritestate, "inactive"))
	//				mypriority = StateSprites::Inactive;
	//			else if (StrUtil::equalCI(myspritestate, "see"))
	//				mypriority = StateSprites::See;
	//			else if (StrUtil::equalCI(myspritestate, "idle"))
	...
	//			if (StrUtil::equalCI(spritestate, "spawn"))
	//				mypriority = StateSprites::Spawn;
	//			else if (StrUtil::equalCI(spritestate, "inactive"))
	//				mypriority = StateSprites::Inactive;
	//			else if (StrUtil::equalCI(spritestate, "see"))
	//				mypriority = StateSprites::See;
	//			else if (StrUtil::equalCI(spritestate, "idle"))
	//				mypriority = StateSprites::Idle;
...
void parseDecorateOld(Tokenizer& tz, std::map<int, ThingType>& types)
{
	...
	do
	{
		...
		// if (StrUtil::equalCI(token, "DoomEdNum"))
		if (tz.checkNC("doomednum"))
		{
			// tz.getInteger(&type);
			type = tz.next().asInt();
		}
		// else if (StrUtil::equalCI(token, "Sprite"))
		else if (tz.checkNC("sprite"))
		{
			sprite      = tz.next().text;
			spritefound = true;
		}
		// else if (StrUtil::equalCI(token, "Frames"))
	...
	if (type > 0)
	{
		...
		Log::info(3, fmt::format("Parsed {} {}: {}", group.length() ? group : "decoration", name, type));
	}
	else
		Log::info(3, fmt::format("Not adding {} {}, no editor number", group.length() ? group : "decoration", name));
}
...
bool Game::readDecorateDefs(Archive* archive, std::map<int, ThingType>& types, vector<ThingType>& parsed)
{
	...
	Log::info(2, fmt::format("Parsing DECORATE entries found in archive {}", archive->filename()));
