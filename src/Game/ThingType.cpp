...
ThingType::ThingType(string_view name, string_view group, string_view class_name) :
	...
	number_{ -1 },
	class_name_{ class_name.data(), class_name.size() }
{
	// Init args
	args_.count   = 0;
	args_[0].name = "Arg1";
...
void ThingType::parse(ParseTreeNode* node)
{
	...
	for (unsigned a = 0; a < node->nChildren(); a++)
	{
		...
		// Sprite
		else if (StrUtil::equalCI(name, "sprite"))
			sprite_ = child->stringValue();

		// Icon
		else if (StrUtil::equalCI(name, "icon"))
			icon_ = child->stringValue();

		// Radius
		else if (StrUtil::equalCI(name, "radius"))
			radius_ = child->intValue();

		// Height
		else if (StrUtil::equalCI(name, "height"))
			height_ = child->intValue();

		// Scale
		else if (StrUtil::equalCI(name, "scale"))
		{
		...
		// ScaleX
		else if (StrUtil::equalCI(name, "scalex"))
			scale_.x = child->floatValue();

		// ScaleY
		else if (StrUtil::equalCI(name, "scaley"))
			scale_.y = child->floatValue();

		// Colour
		else if (StrUtil::equalCI(name, "colour"))
			colour_.set(child->intValue(0), child->intValue(1), child->intValue(2));

		// Show angle
		else if (StrUtil::equalCI(name, "angle"))
			angled_ = child->boolValue();

		// Hanging object
		else if (StrUtil::equalCI(name, "hanging"))
			hanging_ = child->boolValue();

		// Shrink on zoom
		else if (StrUtil::equalCI(name, "shrink"))
			shrink_ = child->boolValue();

		// Fullbright
		else if (StrUtil::equalCI(name, "fullbright"))
			fullbright_ = child->boolValue();

		// Decoration
		else if (StrUtil::equalCI(name, "decoration"))
			decoration_ = child->boolValue();

		// Solid
		else if (StrUtil::equalCI(name, "solid"))
			solid_ = child->boolValue();

		// Translation
		else if (StrUtil::equalCI(name, "translation"))
		{
			translation_ += "\"";
			size_t v = 0;
			do
		...
		// Palette override
		else if (StrUtil::equalCI(name, "palette"))
			palette_ = child->stringValue();

		// Zeth icon
		else if (StrUtil::equalCI(name, "zeth"))
			zeth_icon_ = child->intValue();

		// Pathed things stuff
		else if (StrUtil::equalCI(name, "nexttype"))
		{
			next_type_ = child->intValue();
			flags_ |= FLAG_PATHED;
		}
		else if (StrUtil::equalCI(name, "nextargs"))
		{
		...
		else if (StrUtil::equalCI(name, "player_coop"))
			flags_ |= FLAG_COOPSTART;
		else if (StrUtil::equalCI(name, "player_dm"))
			flags_ |= FLAG_DMSTART;
		else if (StrUtil::equalCI(name, "player_team"))
			flags_ |= FLAG_TEAMSTART;

		// Hexen's critters are weird
		else if (StrUtil::equalCI(name, "dragon"))
			flags_ |= FLAG_DRAGON;
		else if (StrUtil::equalCI(name, "script"))
			flags_ |= FLAG_SCRIPT;
