...
public:
	...
	MapDef   mapInfo(string_view mapname);
	...
	void readUDMFProperties(ParseTreeNode* node, UDMFPropMap& plist) const;
	...
	bool readConfiguration(
		...
		bool        clear       = true);
	bool openConfig(string_view game, string_view port = "", uint8_t format = MAP_UNKNOWN);
	...
	bool   thingFlagSet(const string& udmf_name, MapThing* thing, int map_format);
	bool   thingBasicFlagSet(const string& flag, MapThing* line, int map_format);
	string thingFlagsString(int flags);
	void   setThingFlag(unsigned flag_index, MapThing* thing, bool set = true);
	void   setThingFlag(const string& udmf_name, MapThing* thing, int map_format, bool set = true);
	void   setThingBasicFlag(const string& flag, MapThing* line, int map_format, bool set = true);
	...
	bool        lineFlagSet(const string& udmf_name, MapLine* line, int map_format);
	bool        lineBasicFlagSet(const string& flag, MapLine* line, int map_format);
	string      lineFlagsString(MapLine* line);
	void        setLineFlag(unsigned flag_index, MapLine* line, bool set = true);
	void        setLineFlag(const string& udmf_name, MapLine* line, int map_format, bool set = true);
	void        setLineBasicFlag(const string& flag, MapLine* line, int map_format, bool set = true);
	...
	// Defaults
	string getDefaultString(MapObject::Type type, const string& property);
	int    getDefaultInt(MapObject::Type type, const string& property);
	double getDefaultFloat(MapObject::Type type, const string& property);
	bool   getDefaultBool(MapObject::Type type, const string& property);
...
private:
	...
	int         boom_sector_flag_start_ = 0; // Beginning of Boom sector flags
