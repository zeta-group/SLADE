...
DocsPage::DocsPage(wxWindow* parent) : wxPanel(parent, -1)
{
	...
	wv_browser_->Bind(wxEVT_WEBVIEW_NAVIGATING, [&](wxEvent& e) {
	...
	wv_browser_->Bind(wxEVT_WEBVIEW_LOADED, [&](wxEvent&) { updateNavButtons(); });
}
...
{
	wv_browser_->LoadURL(StrUtil::join(docsUrl(), "?page=", page_name));
}
