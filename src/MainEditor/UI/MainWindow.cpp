...
public:
	...
	bool OnDropFiles(wxCoord x, wxCoord y, const wxArrayString& filenames) override
	{
		for (const auto& filename : filenames)
			App::archiveManager().openArchive(WxUtils::stringToView(filename));
...
MainWindow::MainWindow() : STopWindow("SLADE", "main")
{
	...
	if (mw_maximized)
		wxFrame::Maximize();
	setupLayout();
	wxFrame::SetDropTarget(new MainWindowDropTarget());
}
...
void MainWindow::loadLayout() const
{
	...
	while (true)
	{
		...
		if (!component.empty() && !layout.empty())
			m_mgr_->LoadPaneInfo(layout, m_mgr_->GetPane(component));
...
void MainWindow::saveLayout() const
{
	...
	// Console pane
	file.Write("\"console\" ");
	string pinf = m_mgr_->SavePaneInfo(m_mgr_->GetPane("console")).ToStdString();
	file.Write(fmt::sprintf("\"%s\"\n", pinf));

	// Archive Manager pane
	file.Write("\"archive_manager\" ");
	pinf = m_mgr_->SavePaneInfo(m_mgr_->GetPane("archive_manager"));
	file.Write(fmt::sprintf("\"%s\"\n", pinf));

	// Undo History pane
	file.Write("\"undo_history\" ");
	pinf = m_mgr_->SavePaneInfo(m_mgr_->GetPane("undo_history"));
	file.Write(fmt::sprintf("\"%s\"\n", pinf));
...
void MainWindow::setupLayout()
{
	...
	m_mgr_ = new wxAuiManager(this);
	m_mgr_->SetArtProvider(new SAuiDockArt());
	wxAuiPaneInfo p_inf;
	...
	m_mgr_->AddPane(stc_tabs_, p_inf);
	...
	p_inf.Name("console");
	m_mgr_->AddPane(panel_console, p_inf);
	...
	p_inf.Show(true);
	p_inf.Dock();
	m_mgr_->AddPane(panel_archivemanager_, p_inf);
	...
	p_inf.Show(false);
	p_inf.Dock();
	m_mgr_->AddPane(panel_undo_history_, p_inf);
...
bool MainWindow::exitProgram()
{
	...
	global_palette = palette_chooser_->GetStringSelection().ToStdString();
...
bool MainWindow::handleAction(string_view id)
{
	...
	if (id == "main_about")
	{
		...
		info.SetCopyright(fmt::sprintf("(C) 2008-%s Simon Judd <sirjuddington@gmail.com>", year));
...
{
	// Update toolbar size
	m_mgr_->GetPane(toolbar_).MinSize(-1, toolbar_->minHeight());
	m_mgr_->Update();
}
