	...
	removed_files_{ archive->removedFiles() }
{
	...
	{
		EntryInfo inf;
		inf.file_path     = entry->exProp("filePath").stringValue();
		inf.entry_path    = entry->path(true);
		inf.is_dir        = (entry->type() == EntryType::folderType());
		inf.file_modified = archive->fileModificationTime(entry);
...
wxThread::ExitCode DirArchiveCheck::Entry()
{
	...
	{
		const string& path = info.file_path;
		...
		{
			if (!wxDirExists(path))
				addChange(DirEntryChange(DirEntryChange::DELETED_DIR, path, info.entry_path));
		}
		else
		{
			if (!wxFileExists(path))
				addChange(DirEntryChange(DirEntryChange::DELETED_FILE, path, info.entry_path));
		}
	}
	...
	for (const auto& file : files)
	{
		...
		if (!found)
			addChange(DirEntryChange(DirEntryChange::ADDED_FILE, file, "", mod));
		// Matched, check modification time
		else if (mod > inf.file_modified)
			addChange(DirEntryChange(DirEntryChange::UPDATED, file, inf.entry_path, mod));
	}
	...
	for (const auto& tdir : dirs)
	{
		...
		if (VECTOR_EXISTS(removed_files_, tdir))
			continue;
		...
		// No match, added to archive
		if (!found)
			addChange(DirEntryChange(DirEntryChange::ADDED_DIR, tdir, "", mod));
	}
...
void ArchiveManagerPanel::updateOpenListItem(int index) const
{
	...
	// Set item name
	list_archives_->setItemText(index, 0, fn.fileName());
	list_archives_->setItemText(index, 1, fn.path(false));

	// Set item status colour
	if (archive->canSave())
	{
		if (archive->isModified())
			list_archives_->setItemStatus(index, ListView::ItemStatus::Modified);
		else
			list_archives_->setItemStatus(index, ListView::ItemStatus::Normal);
	}
	else
		list_archives_->setItemStatus(index, ListView::ItemStatus::New);
}
...
void ArchiveManagerPanel::updateRecentListItem(int index) const
{
	...
	// Set item name
	list_recent_->setItemText(index, 0, fn.fileName());
	list_recent_->setItemText(index, 1, fn.path(false));

	// Set item icon
	int  icon = 0;
	auto ext  = fn.extension();
	if (StrUtil::equalCI(ext, "wad"))
		icon = 1;
	else if (StrUtil::equalCI(ext, "zip") || StrUtil::equalCI(ext, "pk3") || StrUtil::equalCI(ext, "pke"))
		icon = 2;
...
void ArchiveManagerPanel::updateArchiveTabTitle(int index) const
{
	...
	for (size_t a = 0; a < stc_archives_->GetPageCount(); a++)
	{
		...
		auto ap = (ArchivePanel*)stc_archives_->GetPage(a);
		if (ap->archive() == archive)
		{
			string title;
			if (archive->isModified())
				title = fmt::sprintf("%s *", archive->filename(false));
			else
...
bool ArchiveManagerPanel::isArchivePanel(int tab_index) const
{
	...
	return stc_archives_->GetPage(tab_index)->GetName().CmpNoCase("archive") == 0;
}
...
bool ArchiveManagerPanel::isEntryPanel(int tab_index) const
{
	...
	// Check the page's name
	return stc_archives_->GetPage(tab_index)->GetName().CmpNoCase("entry") == 0;
}
...
Archive* ArchiveManagerPanel::getArchive(int tab_index) const
{
	...
	auto ap = (ArchivePanel*)stc_archives_->GetPage(tab_index);
	return ap->archive();
}
...
Archive* ArchiveManagerPanel::currentArchive() const
{
	...
	{
		auto ap = (ArchivePanel*)page;
		return ap->archive();
	}

	// EntryPanel
	if (page->GetName() == "entry")
	{
		auto ep = (EntryPanel*)page;
...
EntryPanel* ArchiveManagerPanel::currentArea() const
{
	...
	{
		auto ap = (ArchivePanel*)stc_archives_->GetPage(selected);
...
// -----------------------------------------------------------------------------
// Opens a new tab for the given [archive]
// -----------------------------------------------------------------------------
...
void ArchiveManagerPanel::openTextureTab(int archive_index, ArchiveEntry* entry) const
{
	...
	for (size_t a = 0; a < stc_archives_->GetPageCount(); a++)
	{
		...
		// Check for archive match
		auto txed = (TextureXEditor*)stc_archives_->GetPage(a);
	...
	stc_archives_->AddPage(txed, fmt::sprintf("Texture Editor (%s)", archive->filename(false)), true);
...
bool ArchiveManagerPanel::redirectToTab(ArchiveEntry* entry) const
{
	for (unsigned a = 0; a < stc_archives_->GetPageCount(); a++)
	{
		...
		auto ep = (EntryPanel*)stc_archives_->GetPage(a);
		if (ep->entry() == entry)
		{
...
void ArchiveManagerPanel::openEntryTab(ArchiveEntry* entry) const
{
	...
	ArchivePanel* panel = getArchiveTab(entry->parent());
	...
	stc_archives_->AddPage(ep, fmt::sprintf("%s/%s", entry->parent()->filename(false), entry->name()), true);
...
// -----------------------------------------------------------------------------
void ArchiveManagerPanel::openFile(string_view filename) const
{
	// Show splash screen
	...
	sw.Pause();
	Log::info(fmt::sprintf("Opening took %d ms", (int)sw.Time()));
...
{
	for (const auto& file : files)
		openFile(WxUtils::stringToView(file));
}
...
void ArchiveManagerPanel::openDirAsArchive(string_view dir) const
{
	...
	{
		// If archive didn't open ok, show error message
		wxMessageBox(fmt::sprintf("Error opening directory %s:\n%s", dir, Global::error), "Error", wxICON_ERROR);
	}
}
...
bool ArchiveManagerPanel::undo() const
{
	...
	// Archive panel
	if (page_current->GetName().CmpNoCase("archive") == 0)
	{
	...
	// TEXTUREx panel
	if (page_current->GetName().CmpNoCase("texture") == 0)
	{
	...
	for (int a = 0; a < App::archiveManager().numArchives(); a++)
	{
		...
		if (archive->canSave())
		{
			...
			if (!archive->save())
			{
				...
				wxMessageBox(fmt::sprintf("Error: %s", Global::error), "Error", wxICON_ERROR);
			}
		}
		else
		{
			...
			if (!filename.empty())
			{
				...
				S_SET_VIEW(dir_last.value, fn.path());
			}
		}
	}
}
...
void ArchiveManagerPanel::checkDirArchives()
{
	...
	for (int a = 0; a < App::archiveManager().numArchives(); a++)
	{
		...
		Log::info(2, fmt::sprintf("Checking %s for external changes...", archive->filename()));
...
// -----------------------------------------------------------------------------
void ArchiveManagerPanel::createNewArchive(string_view format) const
{
...
bool ArchiveManagerPanel::beforeCloseArchive(Archive* archive)
{
	...
	if (MapEditor::window()->IsShown() && MapEditor::editContext().hasMapOpen(archive))
	{
	...
	auto txed          = getTextureTab(archive_index);
	if (txed)
	{
	...
	if (archive->isModified() && archive->isWritable())
	{
		...
		wxMessageDialog md(
			this,
			fmt::sprintf("Save changes to archive %s?", archive->filename(false)),
			"Unsaved Changes",
			wxYES_NO | wxCANCEL);
...
vector<int> ArchiveManagerPanel::getSelectedFiles() const
{
	...
	long item = list_recent_->GetNextItem(-1, wxLIST_NEXT_ALL, wxLIST_STATE_SELECTED);
	while (item >= 0)
		ret.push_back(item);
...
vector<int> ArchiveManagerPanel::getSelectedBookmarks() const
{
	...
	long item = list_bookmarks_->GetNextItem(-1, wxLIST_NEXT_ALL, wxLIST_STATE_SELECTED);
	while (item >= 0)
		ret.push_back(item);
...
void ArchiveManagerPanel::saveSelection() const
{
	...
	auto selection = getSelectedArchives();

	// Don't continue if there are no selected items
	if (selection.empty())
		return;

	// Save archives in selection
...
void ArchiveManagerPanel::saveSelectionAs() const
{
	...
	// Save archives in selection as new files
	for (int index : selection)
...
{
	// Get the list of selected list items
	auto selection = getSelectedFiles();
	...
	for (int index : selection)
		App::archiveManager().openArchive(App::archiveManager().recentFile(index));
}
...
bool ArchiveManagerPanel::handleAction(string_view id)
{
	...
	else if (id == "aman_open")
	{
		...
		if (dialog_open.ShowModal() == wxID_OK)
		{
			...
			// Save 'dir_last'
			dir_last.value = dialog_open.GetDirectory();
		}
	}
	...
	else if (id == "aman_opendir")
	{
		...
		wxDirDialog dialog_open(
			this, "Select a Directory to open", dir_last.value, wxDD_DIR_MUST_EXIST | wxDD_NEW_DIR_BUTTON);
		...
		if (dialog_open.ShowModal() == wxID_OK)
		{
			...
			// Open directory
			openDirAsArchive(WxUtils::stringToView(dialog_open.GetPath()));

			wxEndBusyCursor();

			// Save 'dir_last'
			dir_last.value = dialog_open.GetPath();
		}
	}
...
void ArchiveManagerPanel::updateBookmarkListItem(int index) const
{
	...
	// Set item status colour
	if (entry->isLocked())
		list_bookmarks_->setItemStatus(index, ListView::ItemStatus::Locked);
	else
		switch (entry->state())
		{
		case 0: list_bookmarks_->setItemStatus(index, ListView::ItemStatus::Normal); break;
		case 1: list_bookmarks_->setItemStatus(index, ListView::ItemStatus::Modified); break;
		case 2: list_bookmarks_->setItemStatus(index, ListView::ItemStatus::New); break;
		default: list_bookmarks_->setItemStatus(index, ListView::ItemStatus::Error); break;
		}
}
...
{
	auto selection = getSelectedBookmarks();
...
void ArchiveManagerPanel::goToBookmark(long index) const
{
	...
	if (tab->GetName().CmpNoCase("archive") != 0)
		return;
...
void ArchiveManagerPanel::onArchiveTabChanged(wxAuiNotebookEvent& e)
{
	...
	{
		auto ap = (ArchivePanel*)stc_archives_->GetPage(selection);
	...
	{
		auto ep = (EntryPanel*)stc_archives_->GetPage(selection);
		ep->addCustomMenu();
	}
...
void ArchiveManagerPanel::onDirArchiveCheckCompleted(wxThreadEvent& e)
{
	...
	{
		Log::info(2, fmt::sprintf("Finished checking %s for external changes", change_list.archive->filename()));
