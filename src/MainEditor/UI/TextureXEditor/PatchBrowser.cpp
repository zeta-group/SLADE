...
string PatchBrowserItem::itemInfo()
{
	...
	{
		info += ", ";
		info += StrUtil::capitalize(nspace_);
		info += " namespace";
	}
...
PatchBrowser::PatchBrowser(wxWindow* parent) : BrowserWindow(parent)
{
	...
	// Set dialog title
	wxDialog::SetTitle("Browse Patches");
}
...
bool PatchBrowser::openPatchTable(PatchTable* table)
{
	...
	theMainWindow->paletteChooser()->setGlobalFromArchive(table->getParent());
...
bool PatchBrowser::openArchive(Archive* archive)
{
	...
	for (auto entry : patches)
	{
		...
		// Add it
		auto item = new PatchBrowserItem(entry->upperNameNoExt().substr(0, 8), archive, 0, ns);
		addItem(item, nspace.append(entry->parent()->filename(false)));
	}
...
bool PatchBrowser::openTextureXList(TextureXList* texturex, Archive* parent)
{
	...
	{
		// Create browser item
		PatchBrowserItem* item = new PatchBrowserItem(texturex->getTexture(a)->name(), parent, 1);
