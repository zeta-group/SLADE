...
public:
	...
	// Texture operations
	CTexture::UPtr newTextureFromPatch(string_view name, string_view patch);
	void           newTexture();
	...
	void           exportTexture();
	bool           exportAsPNG(CTexture* texture, string_view filename, bool force_rgba) const;
	void           extractTexture();
	bool           modifyOffsets();
	void           moveUp();
	void           moveDown();
	...
	// Undo/Redo
	void onUndo(string_view undo_action);
	void onRedo(string_view undo_action);
