...
public:
	...
	TextureXList::Format selectedFormat() const
	{
...
bool TextureXEditor::openArchive(Archive* archive)
{
	...
	for (auto& entry : tx_entries)
	{
		...
		// Open TEXTUREX entry
		if (tx_panel->openTEXTUREX(entry))
		{
			// Set palette
			...
			tabs_->AddPage(tx_panel, entry->name());
		}
...
// -----------------------------------------------------------------------------
int TextureXEditor::browsePatchTable(string_view first) const
{
...
// -----------------------------------------------------------------------------
string TextureXEditor::browsePatchEntry(string_view first)
{
...
bool TextureXEditor::checkTextures()
{
	...
	for (auto& texture_editor : texture_editors_)
	{
		...
		for (unsigned t = 0; t < texture_editor->txList().nTextures(); t++)
		{
			// Get texture
			CTexture* tex = texture_editor->txList().getTexture(t);
...
{
	if (index < tabs_->GetPageCount() && (int)index != tabs_->GetSelection())
...
void TextureXEditor::setSelection(ArchiveEntry* entry) const
{
	...
	{
		if (StrUtil::equalCI(tabs_->GetPage(a)->GetName().c_str().AsChar(), "pnames") && (entry == pnames_))
		{
			tabs_->SetSelection(a);
			return;
		}
		else if (StrUtil::equalCI(tabs_->GetPage(a)->GetName().c_str().AsChar(), "textures"))
		{
			auto txp = (TextureXPanel*)tabs_->GetPage(a);
		...
		if (dlg.ShowModal() == wxID_YES)
		{
			...
			while (true)
			{
				...
				if (ctxd.createNewSelected())
				{
					...
					auto format = ctxd.selectedFormat();

					// Doom or Strife TEXTUREx
					if (format == TextureXList::Format::Normal || format == TextureXList::Format::Strife11)
					{
						// Create texture list
						TextureXList txlist;
						txlist.setFormat(ctxd.selectedFormat());
					...
					else if (format == TextureXList::Format::Textures)
					{
						// Create texture list
						TextureXList txlist;
						txlist.setFormat(format);
				...
				else
				{
					...
					{
						ArchiveEntry* texturex = archive->addEntry(a, "global", true);
						texturex->setType(EntryType::fromId("texturex"));
						texturex->setExtensionByType();
					}
