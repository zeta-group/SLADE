...
// -----------------------------------------------------------------------------
EntryPanel::EntryPanel(wxWindow* parent, string_view id) : wxPanel(parent, -1), id_{ id.data(), id.size() }
{
...
bool EntryPanel::openEntry(ArchiveEntry* entry)
{
	...
	entry_data_.clear();
	entry_data_.importMem(entry->dataRaw(true), entry->size());
...
// -----------------------------------------------------------------------------
// Saves the entrypanel content to the entry
// (does nothing here, to be overridden by child classes)
// -----------------------------------------------------------------------------
...
void EntryPanel::onToolbarButton(wxCommandEvent& e)
{
	...
	// Revert
		...
		toolbarButtonClick(button.To8BitData().data());
}
