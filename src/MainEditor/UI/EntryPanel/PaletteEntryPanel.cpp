...
public:
		...
		palette_{ pal }
	{
		...
		// Setup preview
		pal_preview_->allowSelection(PaletteCanvas::SelectionType::Range);
	...
	Palette* getFinalPalette() const { return pal_preview_->palette(); }
...
public:
	...
	float getAmount() const { return (float)slider_amount_->GetValue() * 0.01f; }
...
public:
	...
	float getHue() const { return (float)slider_hue_->GetValue() * 0.002f; }

	float getSat() const { return (float)slider_sat_->GetValue() * 0.01f; }

	float getLum() const { return (float)slider_lum_->GetValue() * 0.01f; }

	// Re-apply the changes in selection, hue, saturation and luminosity on a fresh palette
	void redraw() const
	{
		pal_preview_->setPalette(palette_);
		pal_preview_->palette()->shift(getHue(), pal_preview_->getSelectionStart(), pal_preview_->getSelectionEnd());
...
public:
	...
	{
		pal_preview_->setPalette(palette_);
		pal_preview_->palette()->invert(pal_preview_->getSelectionStart(), pal_preview_->getSelectionEnd());
		pal_preview_->draw();
	}
...
public:
	...
	ColRGBA getStartColour() const
	{
		wxColour col = cp_startcolour_->GetColour();
		return { COLWX(col) };
	}

	ColRGBA getEndColour() const
	{
...
PaletteEntryPanel::PaletteEntryPanel(wxWindow* parent) : EntryPanel(parent, "palette")
{
	...
	pal_canvas_->allowSelection(PaletteCanvas::SelectionType::Single);
	...
	toolbar_->addActionGroup("Palette Organisation", StrUtil::split(actions, ';'));

	// Colour Operations
	actions = "ppal_colourise;ppal_tint;ppal_invert;ppal_tweak;ppal_gradient";
	toolbar_->addActionGroup("Colours", StrUtil::split(actions, ';'));

	// Palette Operations
	actions = "ppal_addcustom;ppal_exportas;ppal_importfrom;ppal_test;ppal_generate";
	toolbar_->addActionGroup("Palette Operations", StrUtil::split(actions, ';'));
...
// -----------------------------------------------------------------------------
void PaletteEntryPanel::toolbarButtonClick(string_view action_id)
{
	// Prev. palette
...
bool PaletteEntryPanel::addCustomPalette()
{
	...
	string name = wxGetTextFromUser("Enter name for Palette:", "Add to Palettes").ToStdString();
	if (name.empty())
		return false;
	...
	auto pal = std::make_unique<Palette>();
	pal->copyPalette(palettes_[cur_palette_].get());
...
{
	// Get name to export as
	string name = "Test: " + wxGetTextFromUser("Enter name for Palette:", "Test Palettes").ToStdString();
...
bool PaletteEntryPanel::clearOne()
{
	...
	{
		Log::warning(1, "Palette cannot be removed, no other palette in this entry.");
		return false;
	}
...
bool PaletteEntryPanel::clearOthers()
{
	...
	// Pull the current palette from the list, clear the list then put it back
	auto pal = palettes_[cur_palette_].release();
	palettes_.clear();
	palettes_.emplace_back(pal);
...
{
	auto newpalette = std::make_unique<Palette>();
	newpalette->copyPalette(palettes_[cur_palette_].get());
	palettes_.push_back(std::move(newpalette));
...
bool PaletteEntryPanel::tint()
{
	auto pal = std::make_unique<Palette>();
	pal->copyPalette(palettes_[cur_palette_].get());
	PaletteTintDialog ptd(theMainWindow, pal.get());
...
bool PaletteEntryPanel::colourise()
{
	auto pal = std::make_unique<Palette>();
	pal->copyPalette(palettes_[cur_palette_].get());
	PaletteColouriseDialog pcd(theMainWindow, pal.get());
...
bool PaletteEntryPanel::tweak()
{
	auto pal = std::make_unique<Palette>();
	pal->copyPalette(palettes_[cur_palette_].get());
	PaletteColourTweakDialog pctd(theMainWindow, pal.get());
...
bool PaletteEntryPanel::invert()
{
	auto pal = std::make_unique<Palette>();
	pal->copyPalette(palettes_[cur_palette_].get());
	PaletteInvertDialog pid(theMainWindow, pal.get());
...
bool PaletteEntryPanel::gradient()
{
	auto pal = std::make_unique<Palette>();
	pal->copyPalette(palettes_[cur_palette_].get());
	PaletteGradientDialog pgd(theMainWindow, pal.get());
...
bool PaletteEntryPanel::generateColormaps()
{
	...
	colormap         = entry_->parent()->getEntry("COLORMAP", true);
...
void PaletteEntryPanel::generatePalette(int r, int g, int b, int shift, int steps)
{
	...
	pal->copyPalette(palettes_[0].get());
...
// -----------------------------------------------------------------------------
bool PaletteEntryPanel::handleAction(string_view id)
{
	// Ignore if hidden
	if (!isActivePanel())
		return false;
