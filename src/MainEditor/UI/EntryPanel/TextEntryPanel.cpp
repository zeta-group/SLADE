...
TextEntryPanel::TextEntryPanel(wxWindow* parent) : EntryPanel(parent, "text")
{
	...
	toolbar_->addActionGroup("Scripts", StrUtil::split(custom_toolbar_actions_, ';'));
...
// -----------------------------------------------------------------------------
bool TextEntryPanel::handleAction(string_view id)
{
...
void TextEntryPanel::onChoiceLanguageChanged(wxCommandEvent& e)
{
	...
	TextLanguage* tl = TextLanguage::fromName(choice_text_language_->GetStringSelection().c_str().AsChar());
