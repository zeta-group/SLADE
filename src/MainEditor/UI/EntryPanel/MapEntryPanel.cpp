...
bool MapEntryPanel::loadEntry(ArchiveEntry* entry)
{
	...
	{
		label_stats_->SetLabel(fmt::sprintf(
			...
			map_canvas_->nThings(),
			map_canvas_->totalWidth(),
			map_canvas_->totalHeight()));
		return true;
	}
...
bool MapEntryPanel::createImage()
{
	...
	string     name = fmt::sprintf("%s_%s", entry_->parent()->filename(false), entry_->name());
	wxFileName fn(name);

	// Create save file dialog
	wxFileDialog dialog_save(
		this,
		fmt::sprintf("Save Map Preview \"%s\"", name),
		dir_last.value,
		fn.GetFullName(),
		"PNG (*.PNG)|*.png",
	...
	if (dialog_save.ShowModal() == wxID_OK)
	{
		...
		bool ret = temp.exportFile(dialog_save.GetPath().c_str().AsChar());

		// Save 'dir_last'
		dir_last = dialog_save.GetDirectory().ToStdString();
...
// -----------------------------------------------------------------------------
void MapEntryPanel::toolbarButtonClick(string_view action_id)
{
	// Save Map Image
